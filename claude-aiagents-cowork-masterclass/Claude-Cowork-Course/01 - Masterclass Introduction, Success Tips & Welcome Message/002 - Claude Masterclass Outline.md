...
```mermaid
flowchart LR
    M1["Mô-đun 1<br/>Giới thiệu"] --> M2["Mô-đun 2<br/>Claude Cowork"]
    M2 --> M3["Mô-đun 3<br/>Claude Chat"]
    M3 --> M4["Mô-đun 4<br/>Claude trong Excel"]
    M4 --> M5["Mô-đun 5<br/>Claude trong PowerPoint"]
    M5 --> M6["Mô-đun 6<br/>Claude Code"]
    M6 --> M7["Mô-đun 7<br/>Xây dựng ứng dụng"]
    M7 --> M8["Mô-đun 8<br/>AI Agent và Multi-Agent"]
```
...
```mermaid
flowchart LR
    A["Yêu cầu của người dùng"] --> B["Claude đọc thư mục và dữ liệu"]
    B --> C["Lập kế hoạch công việc"]
    C --> D["Sử dụng skill, plugin hoặc MCP"]
    D --> E["Thực hiện nhiều nhiệm vụ"]
    E --> F["Tổng hợp kết quả"]
    F --> G["Tạo Excel, PowerPoint, PDF hoặc báo cáo"]
```
...
```mermaid
flowchart LR
    A["Dữ liệu thô"] --> B["Kiểm tra chất lượng"]
    B --> C["Làm sạch dữ liệu"]
    C --> D["Sắp xếp và lọc"]
    D --> E["Phân tích"]
    E --> F["Trực quan hóa"]
    F --> G["Thông tin chuyên sâu"]
    G --> H["Quyết định kinh doanh"]
```
...
```mermaid
flowchart TD
    A["Mô tả sản phẩm bằng ngôn ngữ tự nhiên"] --> B["Claude phân tích yêu cầu"]
    B --> C["Lập kế hoạch"]
    C --> D["AI Agent viết hoặc chỉnh sửa mã"]
    D --> E["Chạy ứng dụng"]
    E --> F["Kiểm thử"]
    F --> G{"Đạt yêu cầu?"}
    G -- "Chưa" --> H["Sửa lỗi hoặc cải thiện"]
    H --> D
    G -- "Đạt" --> I["Hoàn thiện sản phẩm"]
```
...
```mermaid
flowchart LR
    A["Người dùng"] --> B["Giao diện nhập thực phẩm"]
    B --> C["Xử lý dữ liệu"]
    C --> D["Tính tổng calo"]
    D --> E["Lưu lịch sử"]
...
```mermaid
flowchart TD
    A["Mục tiêu"] --> B["AI Agent"]
    B --> C["Lập kế hoạch"]
    C --> D["Sử dụng công cụ"]
    D --> E["Thực hiện hành động"]
    E --> F["Quan sát kết quả"]
    F --> G["Lưu vào bộ nhớ"]
    G --> H{"Hoàn thành mục tiêu?"}
    H -- "Chưa" --> C
    H -- "Rồi" --> I["Trả kết quả"]
```
...
```mermaid
flowchart TD
    A["Agent điều phối"] --> B["Agent nghiên cứu"]
    A --> C["Agent phân tích"]
    A --> D["Agent viết nội dung"]
    A --> E["Agent kiểm tra chất lượng"]

    B --> F["Dữ liệu nghiên cứu"]
    C --> G["Kết quả phân tích"]
    D --> H["Bản nội dung"]
    E --> I["Nhận xét và sửa lỗi"]

    F --> J["Kết quả tổng hợp"]
    G --> J
    H --> J
    I --> J
```
...
```mermaid
flowchart LR
    A["Hiểu Claude"] --> B["Viết prompt hiệu quả"]
    B --> C["Làm việc với tệp và dữ liệu"]
    C --> D["Tạo Excel và PowerPoint"]
    D --> E["Điều phối Claude Code"]
    E --> F["Xây dựng ứng dụng"]
    F --> G["Xây dựng AI Agent"]
    G --> H["Tự động hóa quy trình kinh doanh"]
```
