...
```mermaid
flowchart TD
    A[Tải tệp Claude Masterclass Materials] --> B[Quét và kiểm tra tệp]
    B --> C[Giải nén tệp ZIP]
    C --> D[Tạo thư mục chính cho khóa học]
    D --> E[Sao chép tài liệu gốc vào thư mục lưu trữ]
    E --> F[Phân loại theo module]
    F --> G[Đổi tên tệp theo quy ước]
    G --> H[Tạo thư mục thực hành và đầu ra]
    H --> I[Kiểm tra khả năng mở các tệp]
    I --> J[Bắt đầu phần thực hành]
```
...
```mermaid
flowchart LR
    A[Dữ liệu bán hàng mẫu] --> B[Claude Chat]
    A --> C[Excel]
    A --> D[PowerPoint]
    A --> E[Claude Code]
    A --> F[AI Agent]

    B --> B1[Phân tích và đặt câu hỏi]
    C --> C1[Làm sạch và tính toán]
    D --> D1[Tạo báo cáo thuyết trình]
    E --> E1[Xây dựng công cụ xử lý dữ liệu]
    F --> F1[Tự động hóa quy trình]
```
