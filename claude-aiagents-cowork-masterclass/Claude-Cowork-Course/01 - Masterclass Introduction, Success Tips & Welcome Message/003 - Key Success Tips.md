...
```mermaid
flowchart LR
    A[Xem bài giảng] --> B[Tự thực hiện lại]
    B --> C[Thay đổi dữ liệu đầu vào]
    C --> D[Đánh giá kết quả]
    D --> E[Cải thiện prompt hoặc quy trình]
    E --> F[Lưu thành mẫu tái sử dụng]
    F --> B
```
...
```mermaid
flowchart TD
    A[Nhận chủ đề] --> B[Nghiên cứu thông tin]
    B --> C[Xây dựng dàn ý]
    C --> D[Viết nội dung từng slide]
    D --> E[Tạo slide]
    E --> F[Kiểm tra bố cục]
    F --> G[Sửa nội dung và thiết kế]
    G --> H[Xuất bản trình bày hoàn chỉnh]
```
...
```mermaid
flowchart LR
    A[Claude tạo đầu ra] --> B[Kiểm tra tính chính xác]
    B --> C[Đánh giá mức độ phù hợp]
    C --> D[Chỉnh sửa prompt]
    D --> E[Tạo phiên bản mới]
...
```mermaid
flowchart LR
    A[Học kiến thức] --> B[Xây dựng dự án]
    B --> C[Ghi lại quy trình]
    C --> D[Chia sẻ trên LinkedIn]
    D --> E[Nhận phản hồi]
    E --> F[Cải thiện sản phẩm]
    F --> G[Mở rộng mạng lưới nghề nghiệp]
```
...
flowchart LR
    ...
    C --> D[Phát hiện lỗi]
    D --> E[Cải thiện]
    E --> F[Lưu thành thư viện]
    F --> G[Xây dựng portfolio]
    G --> H[Chia sẻ sản phẩm]
```
