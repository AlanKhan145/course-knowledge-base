# 009 - Data Quality

**Hoc phan:** 05 - Serving, Security and Portfolio
**Module:** Module 12 - Security and Data Governance
**Nhom noi dung:** Data Governance
**Nguon roadmap:** Security and Data Governance / Data Governance
**Loai bai:** quality
**Thu tu trong module:** 009
**Thoi luong goi y:** 28 phut

---

## 1. Tom tat

Bai nay giai thich **Data Quality** trong boi canh Data Engineer Roadmap 2026. Sau bai hoc, ban nen biet topic nay nam o dau trong pipeline, anh huong den storage/processing/serving nhu the nao va co the bien thanh artifact trong portfolio.

## 2. Muc tieu hoc tap

- Giai thich duoc Data Quality bang ngon ngu cua ban.
- Nhan biet topic nay lien quan den do tin cay, chi phi, chat luong du lieu, bao mat hoac kha nang mo rong nao.
- Ap dung vao mot pipeline nho bang query, script, diagram, config, checklist hoac README.

## 3. Khai niem chinh

- Data quality means data is accurate, complete, timely, consistent and fit for use.
- Checks should run close to each pipeline step so failures are caught before users depend on bad data.
- Good checks combine schema tests, null checks, duplicate checks, range checks and business rules.

## 4. Thuc hanh

1. Write a 5-line note explaining Data Quality.
2. Add one example from a real data pipeline.
3. Create a tiny artifact: query, diagram, script, checklist or README section.

## 5. Bai tap

Turn this concept into one automated check with pass/fail criteria and an alert owner.

## 6. Checklist hoan thanh

- [ ] Co dinh nghia ngan gon.
- [ ] Co vi du trong pipeline Data Engineer.
- [ ] Co artifact nho de dua vao portfolio.
- [ ] Co ghi chu ve loi thuong gap, monitoring, testing hoac security neu lien quan.

## 7. Ghi chu san xuat

Khi dua vao production, hay hoi: du lieu vao tu dau, schema co on dinh khong, pipeline co idempotent khong, khi fail thi retry/backfill ra sao, ai nhan alert va nguoi dung cuoi se bi anh huong the nao.
