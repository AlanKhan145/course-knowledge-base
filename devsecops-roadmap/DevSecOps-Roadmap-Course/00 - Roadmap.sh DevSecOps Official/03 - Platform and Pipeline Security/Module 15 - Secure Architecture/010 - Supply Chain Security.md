...
Bai nay giai thich **Supply Chain Security** trong boi canh DevSecOps hien dai. Sau bai hoc, ban nen biet topic nay giam risk nao, nam o dau trong lifecycle va co the ap dung vao code, pipeline, cloud, runtime hay incident response.

## 2. Muc tieu hoc tap

- Giai thich duoc Supply Chain Security bang ngon ngu cua ban.
...
- Supply chain security bao ve dependency, package registry, runner, artifact, image, signing va provenance.
- Rui ro co the den tu maintainer account, typosquatting, compromised package hoac build runner.
- Can lockfile, SCA, provenance, artifact signing va secret scanning.
...
- Toi co the giai thich **Supply Chain Security** trong 1-2 phut.
...
**Supply Chain Security** la mot moc trong lo trinh DevSecOps. Hay bien no thanh mot checklist, script, config, scan, detection rule, dashboard hoac runbook de kien thuc co cho bam.
