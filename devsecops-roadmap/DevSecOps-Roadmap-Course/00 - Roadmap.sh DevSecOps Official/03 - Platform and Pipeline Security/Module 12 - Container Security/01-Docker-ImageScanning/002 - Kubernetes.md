...
Bai nay giai thich **Kubernetes** trong boi canh DevSecOps hien dai. Sau bai hoc, ban nen biet topic nay giam risk nao, nam o dau trong lifecycle va co the ap dung vao code, pipeline, cloud, runtime hay incident response.
...
- Kubernetes security gom RBAC, namespace, NetworkPolicy, admission control, secret va pod security.
- Workload nen co request/limit, probe, image policy va service account rieng.
- Cluster can logging, audit, patching va policy enforcement.
...
**Kubernetes** la mot moc trong lo trinh DevSecOps. Hay bien no thanh mot checklist, script, config, scan, detection rule, dashboard hoac runbook de kien thuc co cho bam.
