...
```mermaid
flowchart TD
    A[The 5 Golden Rules of Show, Don't Tell] --> B[1. Use concrete details]
    A --> C[2. Focus on specifics]
    A --> D[3. Evoke emotions]
    A --> E[4. Use the five senses]
    A --> F[5. Use rhetorical figures]

    B --> B1[Choose details the reader can see or imagine]
    C --> C1[Avoid generic description]
    D --> D1[Make emotion rise from the scene]
    E --> E1[Sight, smell, hearing, touch, taste]
    F --> F1[Use metaphor, metonymy, and synaesthesia]
```
...
```mermaid
flowchart TD
    A[Generic Event] --> B[There was an explosion]
    B --> C[Reader receives information]

    D[Specific Experience] --> E[Wrong light]
    D --> F[Chemical fog]
    D --> G[Burning throat]
    D --> H[Chewing gum tastes like dirt]
    D --> I[Head throbs]
    D --> J[Reader experiences confusion and shock]
```
...
```mermaid
flowchart TD
    A[Scene Description] --> B[Sight]
    A --> C[Smell]
    A --> D[Hearing]
    A --> E[Touch]
    A --> F[Taste]

    B --> B1[Color, shape, movement, light]
    C --> C1[Smoke, bread, perfume, decay]
    D --> D1[Voices, silence, footsteps, wind]
    E --> E1[Heat, cold, pressure, texture]
    F --> F1[Bitterness, sweetness, dust, blood]
```
...
```mermaid
flowchart LR
    A[Concrete Details] --> B[Specific Images]
    B --> C[Sensory Experience]
    C --> D[Emotion]
    D --> E[Reader Engagement]
    E --> F[Memorable Scene]

    G[Rhetorical Figures] --> B
    G --> D
```
