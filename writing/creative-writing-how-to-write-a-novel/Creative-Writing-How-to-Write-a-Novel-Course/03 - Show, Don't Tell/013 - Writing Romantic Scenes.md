...
```mermaid
flowchart TD
    A[Know the Characters] --> B[Define Emotional Stakes]
    B --> C[Build Romantic Tension]
    C --> D[Add Barriers or Complications]
    D --> E[Choose One Point of View]
    E --> F[Use Specific Sensory Details]
    F --> G[Show Vulnerability and Change]
    G --> H[Let the Scene Affect the Relationship]
```
...
```mermaid
flowchart LR
    A[Before the Scene: Tension] --> B[Trigger: Forced Closeness or Choice]
    B --> C[Escalation: Subtext and Physical Detail]
    C --> D[Vulnerability: Fear, Desire, or Confession]
    D --> E[Turning Point: Action or Restraint]
    E --> F[Aftermath: Relationship Changes]
```
