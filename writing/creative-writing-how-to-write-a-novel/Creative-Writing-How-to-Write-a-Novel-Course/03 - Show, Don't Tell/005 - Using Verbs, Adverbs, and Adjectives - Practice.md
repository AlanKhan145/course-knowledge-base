...
```mermaid
flowchart TD
    A[Word Choice in Show, Don't Tell] --> B[Verbs]
    A --> C[Adverbs]
    A --> D[Adjectives]

    B --> B1[Choose precise, concrete actions]
    B --> B2[Prefer active voice]
    B --> B3[Avoid unnecessary filter verbs]

    C --> C1[Use with caution]
    C --> C2[Often replace with stronger verbs]
    C --> C3[Can reveal weak description]

    D --> D1[Use only when necessary]
    D --> D2[Add precision, not decoration]
    D --> D3[Avoid clichés and vague intensifiers]
```
...
```mermaid
flowchart LR
    A[Character experiences something] --> B[Filter Verb]
    B --> C[Reader receives reported experience]

    D[Character experiences something] --> E[Direct Description]
    E --> F[Reader experiences the scene more immediately]
```
...
```mermaid
flowchart TD
    A[Sentence with adverb] --> B{Is the adverb necessary?}

    B -->|Yes| C[Keep it if it adds precision]
    B -->|No| D[Replace it]

    D --> E[Use a stronger verb]
    D --> F[Use action]
    D --> G[Use body language]
    D --> H[Use sensory detail]
```
