...
```mermaid
flowchart TD
    A[Abstract Idea] --> B[Ask: What can be seen?]
    B --> C[Choose a physical detail]
    C --> D[Add sensory information]
    D --> E[Connect image to emotion or theme]
    E --> F[Vivid scene in the reader's mind]

    A1[She is sad] --> B1[What does sadness look like?]
    B1 --> C1[Unopened letter in her hands]
    C1 --> D1[Paper creased, silence in the room]
    D1 --> E1[Loss, hesitation, grief]
    E1 --> F1[Reader feels the sadness without being told]
```
