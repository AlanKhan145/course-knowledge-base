...
```mermaid
flowchart TD
    A[Character Trait] --> B[Emotional Reaction]
    A --> C[Physical Detail]
    A --> D[Behavior and Action]
    A --> E[Dialogue and Voice]
    A --> F[Other Characters' Reactions]

    B --> G[Reader Infers Personality]
    C --> G
    D --> G
    E --> G
    F --> G
...
```mermaid
flowchart LR
    A[First Meeting] --> B[Surface Details]
    B --> C[Voice and Behavior]
    C --> D[Contradictions]
    D --> E[Deeper Character Truth]
```
...
```mermaid
flowchart TD
    A[Choose a Trait] --> B[Remove Direct Adjective]
    B --> C[Find a Visible Behavior]
    C --> D[Add Emotional or Physical Detail]
    D --> E[Let Another Character React]
    E --> F[Trust the Reader to Infer]
```
