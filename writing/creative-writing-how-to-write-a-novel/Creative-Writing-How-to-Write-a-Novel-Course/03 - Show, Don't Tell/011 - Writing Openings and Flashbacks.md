...
```mermaid
flowchart TD
    A[Do you have a prologue?] --> B{Does the reader need this information early?}

    B -->|No| C[Remove it]
    B -->|Yes| D{Can the information appear naturally later?}

    D -->|Yes| E[Move it into a later scene]
    D -->|No| F{Does it create tone, conflict, theme, or mystery?}

    F -->|No| C
    F -->|Yes| G[Keep it short and focused]

    G --> H[Make sure it supports the main story]
```
...
```mermaid
flowchart LR
    A[Present Scene] --> B[Trigger]
    B --> C[Flashback]
    C --> D[New Understanding]
    D --> E[Return to Present Scene]
    E --> F[Present Conflict Becomes Stronger]
```
