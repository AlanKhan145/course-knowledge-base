...
```mermaid
flowchart TD
    A[Fight Scene] --> B[Physical Conflict]
    A --> C[Emotional Pressure]
    A --> D[Character Revelation]
    A --> E[Plot Consequence]

    B --> B1[Blows, weapons, movement]
    C --> C1[Fear, anger, desperation, hesitation]
    D --> D1[Strengths and weaknesses]
    E --> E1[Victory, injury, escape, loss, new problem]
```
...
```mermaid
flowchart LR
    A[Short Sentences] --> B[Speed]
    B --> C[Panic]
    C --> D[Impact]

    E[Longer Sentences] --> F[Slow Motion]
    F --> G[Realization]
    G --> H[Emotional Weight]
```
...
```mermaid
flowchart TD
    A[Character Personality] --> B[Fighting Style]

    B --> C[Cold and Calculating]
    C --> C1[Precise, efficient, controlled]

    B --> D[Angry and Reckless]
    D --> D1[Wild, risky, forceful]

    B --> E[Afraid but Determined]
    E --> E1[Defensive, desperate, persistent]

    B --> F[Experienced Fighter]
    F --> F1[Uses timing, distance, weak points]

    B --> G[Untrained Fighter]
    G --> G1[Messy, instinctive, emotional]
```
...
```mermaid
flowchart TD
    A[Before the Fight] --> B[Character Goal]
    B --> C[First Clash]
    C --> D[Escalation]
    D --> E[Turning Point]
    E --> F[Injury or Cost]
    ...
    A --> A1[Where are they?]
    B --> B1[What does the character want?]
    C --> C1[Who has the advantage?]
    D --> D1[How does danger increase?]
    E --> E1[What changes the fight?]
    F --> F1[What is lost or damaged?]
    G --> G1[How does the fight end?]
    H --> H1[How does the story move forward?]
```
