...
```mermaid id="v8jwep"
flowchart TD
    A[Why I Write] --> B[Emotional Purpose]
    B --> C[Story Theme]
    C --> D[Character Choices]
    D --> E[Scene Images]
    E --> F[Writing Style]

    A1[Example: I write to understand loneliness] --> B1[Emotional Purpose: make isolation felt]
    B1 --> C1[Theme: people need connection]
    C1 --> D1[Character: someone afraid to be vulnerable]
    D1 --> E1[Images: empty rooms, unread messages, distant lights]
    E1 --> F1[Style: quiet, reflective, sensory]
```
