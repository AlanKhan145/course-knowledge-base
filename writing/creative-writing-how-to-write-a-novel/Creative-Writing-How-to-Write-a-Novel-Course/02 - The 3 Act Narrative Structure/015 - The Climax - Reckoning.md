...
```mermaid
flowchart LR
    A[Act 1: The Lie and the Want] --> B[Act 2: Tests and Transformation]
    B --> C[Second Turning Point]
    C --> D[Act 3: Final Obstacles]
    D --> E[Climax / Reckoning]
    E --> F[Resolution]

    E:::climax

    classDef climax fill:#fee2e2,stroke:#dc2626,stroke-width:2px,color:#000;
```
...
```mermaid
flowchart TD
    A[Strong Climax] --> B[Inevitability]
    A --> C[Unpredictability]

    B --> B1[The ending feels earned]
    B --> B2[The arc pays off]
    B --> B3[The conflict resolves naturally]

    C --> C1[The exact outcome surprises the reader]
    C --> C2[The final choice has tension]
    C --> C3[The story avoids feeling obvious]
```
...
```mermaid
flowchart TD
    A[Final Confrontation] --> B[Highest Stakes]
    B --> C[Impossible Choice]
    C --> D[Peak Moment]
    D --> E[Conflict Resolved]
    E --> F[New Self Confirmed]

    D:::peak

    classDef peak fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#000;
```
...
```mermaid
flowchart LR
    A[Old Self] --> B[Cannot Resolve the Conflict]
    B --> C[Transformation Arc]
    C --> D[New Self]
    D --> E[Secret Weapon]
    E --> F[Climax Resolved]
    ...
    classDef weapon fill:#dcfce7,stroke:#16a34a,stroke-width:2px,color:#000;
```
...
```mermaid
flowchart TD
    A[Main Conflict Seems Resolved] --> B[False Climax]
    B --> C[New Final Problem Appears]
    C --> D[True Climax]
    D --> E[Real Resolution]
```
...
```mermaid
flowchart TD
    A[Act 3 Builds Toward Final Conflict] --> B[The Stakes Reach Their Highest Point]
    B --> C[The Protagonist Faces the Antagonistic Force]
    C --> D[The Old Self Would Fail]
    D --> E[The Protagonist Uses the Lesson of the Arc]
    E --> F[Peak Moment]
    F --> G[Central Conflict Resolved]
    G --> H[Consequences Become Clear]
```
