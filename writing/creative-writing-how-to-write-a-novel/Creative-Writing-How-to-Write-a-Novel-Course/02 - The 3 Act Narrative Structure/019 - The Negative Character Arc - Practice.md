...
```mermaid id="dqe5gx"
flowchart TD
    A[Character Begins with a Lie or Flaw] --> B[Inciting Incident Creates Pressure]
    B --> C[Truth Becomes Available]
    C --> D[Character Rejects the Truth]
    D --> E[Bad Choices Escalate]
    E --> F[Point of No Return]
    F --> G[Character Falls]
    G --> H[World Ends Worse Than Before]
```
...
```mermaid id="8n37ft"
flowchart LR
    A[Believes a Lie] --> B[Faces Conflict]
    B --> C[Discovers the Truth]
    C --> D[Changes for the Better]
```
...
flowchart LR
    A[Believes a Lie] --> B[Faces Conflict]
    B --> C[Sees the Truth]
    C --> D[Rejects the Truth]
    D --> E[Falls Deeper into the Lie]
```
...
```mermaid id="y9j4tp"
flowchart TD
    A[Midpoint Revelation] --> B{How Does the Character Respond?}
    B --> C[Accepts the Truth]
    C --> D[Positive Arc]
    B --> E[Rejects the Truth]
    E --> F[Negative Arc]
```
...
```mermaid id="f1mnyk"
flowchart TD
    A[Character Begins with a Hopeful Lie] --> B[They Search for Truth]
    B --> C[They Discover Reality]
    C --> D[Truth Is Worse Than Expected]
    D --> E[Character Becomes Disillusioned]
```
...
```mermaid id="cy8myv"
flowchart TD
    A[Character Begins with a Lie] --> B[Conflict Challenges Them]
    B --> C[Truth Is Offered]
    C --> D[Character Rejects Truth]
    D --> E[Character Believes Lie More Strongly]
    E --> F[Fall Becomes Inevitable]
```
...
```mermaid id="lxrv1s"
flowchart TD
    A[Character Begins in a Positive or Stable World] --> B[Pressure or Crisis Appears]
    B --> C[Character Makes a Wrong Choice]
    C --> D[Wrong Choice Brings Power or Reward]
    D --> E[Character Chooses the Lie Again]
    E --> F[Morality Decays]
    F --> G[Character Becomes Corrupted]
```
...
```mermaid id="z43eq2"
flowchart TD
    A[Act 1] --> B[Introduce the Character]
    B --> C[Reveal the Lie or Flaw]
    C --> D[Create the Goal]
    D --> E[Launch the Conflict]
```
...
```mermaid id="01rt77"
flowchart TD
    A[Act 2] --> B[Conflict Intensifies]
    B --> C[Truth Becomes Visible]
    C --> D[Character Rejects the Truth]
    D --> E[Bad Choices Escalate]
    E --> F[Point of No Return]
```
...
```mermaid id="lkjl72"
flowchart TD
    A[Act 3] --> B[Final Test]
    B --> C[Character Chooses the Lie Again]
    C --> D[Consequences Arrive]
    D --> E[Fall Is Completed]
    E --> F[Tragic or Dark Ending]
```
...
```mermaid id="m57gwa"
flowchart LR
    A[Small Compromise] --> B[Bigger Lie]
    B --> C[Self-Justification]
    C --> D[Harmful Action]
...
```mermaid id="r9zt5d"
flowchart TD
    A[Character Believes a Lie] --> B[Character Wants the Wrong Thing]
    B --> C[Conflict Reveals the Truth]
    C --> D[Character Rejects the Truth]
    D --> E[Character Chooses the Lie Again]
    E --> F[Choices Become More Destructive]
    F --> G[Point of No Return]
    G --> H[Final Fall]
    H --> I[Tragic or Dark Ending]
```
