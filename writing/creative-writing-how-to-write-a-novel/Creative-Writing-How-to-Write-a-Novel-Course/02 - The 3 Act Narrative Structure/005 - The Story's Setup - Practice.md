...
```mermaid id="p98m4e"
flowchart TD
    A[Story Setup] --> B[Protagonist]
    A --> C[Ordinary World]
    A --> D[Current Desire or Lack]
    A --> E[Stakes]
    A --> F[Early Conflict]
    A --> G[Inciting Event]
```
...
```mermaid id="vf1f1s"
flowchart LR
    A[Act 1 Begins] --> B[Opening Hook]
    B --> C[Introduce Protagonist]
    C --> D[Establish Ordinary World]
    D --> E[Reveal Stakes]
    E --> F[Inciting Event]
    F --> G[Main Story Begins]
```
...
```mermaid id="pwo8l8"
flowchart TD
    A[Ordinary World] --> B[Normal Routine]
    A --> C[Existing Problems]
    A --> D[Relationships]
    A --> E[Beliefs and Values]
    A --> F[Current Limits]
```
...
```mermaid id="hl9s9e"
flowchart LR
    A[Setup: Before Version] --> B[Conflict Tests the Protagonist]
    B --> C[Midpoint Realization]
    C --> D[Final Choice]
    D --> E[Ending: Changed Version]
```
...
```mermaid id="c6a7cu"
flowchart TD
    A[Setup Detail] --> B{Does it pay off later?}
    B -->|Yes| C[Keep it]
    B -->|No| D{Does it reveal character or tone strongly?}
    D -->|Yes| E[Maybe keep it]
    D -->|No| F[Trim it]
```
...
```mermaid id="psftoc"
flowchart LR
    A[Beginning of the Story] --> B[Introduce the Protagonist]
    B --> C[Show the Ordinary World]
    C --> D[Stage the Inciting Event]
```
...
```mermaid id="osfgs8"
flowchart TD
    A[Storm on Bald Mountain] --> B[Juan repairs old relay system]
    B --> C[His skill and values are revealed]
    C --> D[He finds Marcus Green's name]
    D --> E[Mystery begins]
```
...
```mermaid id="ga11ab"
flowchart TD
    A[Wounded Girl Wakes Up] --> B[Finds Pocket Watch]
    B --> C[Reader Asks What Happened]
    C --> D[She Leaves the Building]
    D --> E[Story Needs Clearer Direction]
```
...
```mermaid id="i6qy4t"
flowchart TD
    A[Review Setup] --> B{Is the protagonist clear?}
    B -->|Yes| C{Is the ordinary world shown?}
    B -->|No| D[Introduce the protagonist earlier]

    C -->|Yes| E{Is there tension or movement?}
    C -->|No| F[Add setting, routine, or social context]

    E -->|Yes| G{Is there an inciting event or movement toward one?}
    E -->|No| H[Add conflict, pressure, or a choice]

    G -->|Yes| I{Can anything be cut?}
    G -->|No| J[Move closer to disruption]

    I -->|Yes| K[Trim unnecessary exposition]
    I -->|No| L[Setup is focused]
```
