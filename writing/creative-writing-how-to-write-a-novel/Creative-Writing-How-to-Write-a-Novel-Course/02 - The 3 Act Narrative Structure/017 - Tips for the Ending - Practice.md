...
```mermaid
flowchart LR
    A[Story Questions] --> B[Conflict and Character Arc]
    B --> C[Climax]
    C --> D[Resolution]
    D --> E[Satisfying Ending]

    E --> F[Emotional Closure]
    E --> G[Thematic Resonance]
    E --> H[Memorable Final Image]
```
...
```mermaid
flowchart TD
    A[Did the protagonist reach the goal?] --> B[Yes]
    A --> C[Yes, but at a cost]
    A --> D[No]
    A --> E[They could have, but chose not to]

    B --> B1[Happy Ending]
    C --> C1[Bittersweet Ending]
    D --> D1[Tragic Ending]
    E --> E1[Transformational or Twist Ending]
```
...
```mermaid
flowchart LR
    A[Goal Achieved] --> B[Victory]
    A --> C[Cost Paid]
    B --> D[Bittersweet Ending]
    C --> D
```
...
```mermaid
flowchart TD
    A[Original Goal] --> B[Protagonist Changes]
    B --> C[New Understanding]
    C --> D[Goal Is Re-evaluated]
    D --> E[Protagonist Chooses Not to Take It]
    E --> F[Transformation Confirmed]
```
...
```mermaid
flowchart LR
    A[Act 1: Questions Introduced] --> B[Act 2: Questions Developed]
    B --> C[Act 3: Major Questions Resolved]
    C --> D[Resolution: Emotional Closure]

    A -.Too many unanswered threads.-> C
```
...
```mermaid
flowchart TD
    A[Conflict] --> B[Protagonist's Choices]
    B --> C[Earned Consequences]
    C --> D[Satisfying Ending]

    A -.Random Luck.-> E[Unsatisfying Shortcut]
```
...
```mermaid
flowchart TD
    A[Strong Ending] --> B[Inevitable in Hindsight]
    A --> C[Not Predictable in Advance]

    B --> B1[The clues were there]
    B --> B2[The character arc points toward it]
    B --> B3[The theme supports it]

    C --> C1[The exact outcome still surprises]
    C --> C2[The final choice creates tension]
    C --> C3[The reader stays emotionally engaged]
```
...
```mermaid
flowchart LR
    A[Draft Several Endings] --> B[Compare Their Meaning]
    B --> C[Test With Readers]
    C --> D[Wait and Re-read]
    D --> E[Choose Final Ending]
```
...
```mermaid
flowchart TD
    A[Central Story Question] --> B[Climax]
    B --> C[Protagonist's Final Choice]
    C --> D[Goal Outcome]
    D --> E[Cost or Reward]
    E --> F[Subplot Closure]
    F --> G[Theme Echo]
    G --> H[Final Image or Final Line]
```
