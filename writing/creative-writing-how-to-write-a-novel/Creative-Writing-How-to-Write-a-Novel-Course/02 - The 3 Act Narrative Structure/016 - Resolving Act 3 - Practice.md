...
```mermaid id="5g6d2a"
flowchart LR
    A[Act 1: Questions Raised] --> B[Act 2: Conflict Deepens]
    B --> C[Act 3: Climax]
    C --> D[Resolution]
    D --> E[New State of the World]

    D:::resolution

    classDef resolution fill:#dcfce7,stroke:#16a34a,stroke-width:2px,color:#000;
```
...
```mermaid id="5b3pja"
flowchart TD
    A[Climax] --> B[Main Conflict Resolved]
    B --> C[Emotional Release]
    C --> D[Consequences Revealed]
    D --> E[Final Goodbye]
```
...
```mermaid id="5g0vr9"
flowchart TD
    A[Strong Resolution] --> B[Closure]
    A --> C[Theme]
    A --> D[Detachment]
    A --> E[Final Farewell]
    A --> F[Continuity]

    B --> B1[The main conflict feels complete]
    C --> C1[The deeper meaning returns]
    D --> D1[The pacing softens]
    E --> E1[The reader says goodbye]
    F --> F1[Life continues beyond the book]
```
...
```mermaid id="3cyf1p"
flowchart LR
    A[External Struggle] --> B[Physical Defeat]
    B --> C[Inner Dignity]
    C --> D[Theme Reaffirmed]
```
...
```mermaid id="ut8rn0"
flowchart LR
    A[Story Conflict Ends] --> B[New Life Begins]
    B --> C[Reader Imagines the Future]
```
...
```mermaid id="bl8xal"
flowchart TD
    A[Climax Ends] --> B[Immediate Consequences]
    B --> C[Main Plot Closure]
    C --> D[Subplot Closure]
    D --> E[Proof of Transformation]
    E --> F[Theme Echo]
    F --> G[Final Image]
    G --> H[Sense of Life Continuing]
```
...
```mermaid id="yv5y5y"
flowchart LR
    A[Opening Image] --> B[Story Transformation]
    B --> C[Final Image]
