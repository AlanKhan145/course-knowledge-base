...
flowchart LR
    ...
    B --> C[Act 2A: Exploration and Mistakes]
    C --> D[Midpoint]
    D --> E[Act 2B: Proactive Conflict]
    E --> F[Act 3: Climax and Resolution]
...
```mermaid
flowchart TD
    A[Old Self] --> B[False Assumptions]
    B --> C[Midpoint Revelation]
    C --> D[Self-Reflection]
    D --> E[New Awareness]
    E --> F[New Plan]
    F --> G[Proactive Action]

    C:::important
    D:::important

    classDef important fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#000;
```
...
```mermaid
flowchart LR
    A[Reactive Protagonist] --> B[Midpoint Revelation]
    B --> C[Proactive Protagonist]

    A --> A1[Responds to events]
    A --> A2[Acts from false assumptions]
    A --> A3[Still tied to old self]

    C --> C1[Makes the first move]
    C --> C2[Creates a new plan]
    C --> C3[Moves toward transformation]
```
...
```mermaid
flowchart TD
    A[The protagonist enters the new world] --> B[They make mistakes]
    B --> C[They discover what does not work]
    C --> D[The midpoint event occurs]
    D --> E[The truth is revealed]
    E --> F[The protagonist reflects]
    F --> G[The plan changes]
    G --> H[The protagonist becomes more proactive]
```
