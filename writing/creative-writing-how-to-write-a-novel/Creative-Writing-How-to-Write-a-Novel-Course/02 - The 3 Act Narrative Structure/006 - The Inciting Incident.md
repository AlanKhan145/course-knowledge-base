...
```mermaid
flowchart LR
    A[Ordinary World / Setup] --> B[Inciting Incident]
    B --> C[Reaction and Pressure]
    C --> D[First Turning Point]
    D --> E[Act 2: Confrontation]
    ...
    classDef setup fill:#f4f4f4,stroke:#555,stroke-width:1px;
...
```mermaid
flowchart TD
    A[Stable Life Before the Story] --> B[Inciting Incident]
    B --> C[The Protagonist Reacts]
    C --> D[New Problem or Goal Appears]
    D --> E[Pressure Increases]
    E --> F[First Turning Point]
    F --> G[The Protagonist Enters the Main Conflict]
```
