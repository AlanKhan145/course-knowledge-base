...
flowchart LR
    ...
    B --> C[Act 2: Confrontation]
    C --> D[Midpoint]
    D --> E[Escalating Obstacles]
    E --> F[Act 3: Resolution]
    F --> G[Climax]
    G --> H[Conclusion]
```
...
```mermaid
flowchart TD
    A[Hero pursues goal] --> B[Obstacles increase]
    B --> C[Hero reflects internally]
    C --> D[Midpoint realization]
    D --> E[Conflict becomes more intense]
```
...
```mermaid
timeline
    title Three-Act Narrative Structure
    Act 1 : Introduce hero
          : Establish world
          : Present goal
          : Inciting incident
    Act 2 : Hero pursues goal
          : Obstacles grow
          : Inner conflict deepens
          : Midpoint realization
    Act 3 : Hero takes offensive action
          : Final obstacles
          : Climax
          : Worthy conclusion
```
...
```mermaid
flowchart TD
    A[Does the beginning introduce the hero clearly?] --> B{Is there an inciting incident?}
    B -->|Yes| C[Check Act 2]
    B -->|No| D[Strengthen the event that starts the main conflict]

    C --> E{Does the middle escalate?}
    E -->|Yes| F[Check Act 3]
    E -->|No| G[Add stronger obstacles and turning points]

    F --> H{Does the climax test the changed hero?}
    H -->|Yes| I[Refine the ending]
    H -->|No| J[Connect the climax to the hero's arc]
```
