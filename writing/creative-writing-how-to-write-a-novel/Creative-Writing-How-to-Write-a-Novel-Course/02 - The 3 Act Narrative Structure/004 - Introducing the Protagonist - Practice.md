...
```mermaid id="9j381k"
flowchart LR
    A[Opening Hook] --> B[Reader Becomes Curious]
    B --> C[Protagonist Appears]
    C --> D[Reader Asks: Why This Person?]
    D --> E[Story Builds Attachment]
```
...
```mermaid id="vc0kqa"
flowchart TD
    A[Protagonist Introduction] --> B[Name]
    A --> C[Age or Life Stage]
    A --> D[Occupation or Social Role]
    A --> E[Important Physical Traits]
    A --> F[Story Role]
    A --> G[Personality]
    A --> H[Sympathetic or Interesting Traits]
    A --> I[Goals]
    A --> J[Conflict]
    A --> K[Desires]
    A --> L[Beliefs and Values]
```
...
```mermaid id="9ujg90"
flowchart TD
    A[Jane Eyre Introduction] --> B[Reading Alone]
    B --> C[Introversion and Inner Life]
    A --> D[Conflict with John Reed]
    D --> E[Courage, Passion, Emotional Wound]
```
...
```mermaid id="gypdbf"
flowchart LR
    A[Andy Plays with Woody] --> B[Woody Feels Valued]
    B --> C[Andy Leaves]
    C --> D[Woody Leads the Toys]
    D --> E[Leadership + Insecurity]
```
...
```mermaid id="i8sx6h"
flowchart TD
    A[Dr. Alan Grant Introduction] --> B[Paleontology Expert]
    A --> C[Dislikes Children]
    A --> D[Uncomfortable with Modern Life]
    A --> E[Shy and Isolated]
    E --> F[Later Forced to Protect Children]
```
...
```mermaid id="eky5eg"
flowchart LR
    A[Calm Character Introduction] --> B[Reader Understands Character]
    B --> C[Reader Cares About Them]
    C --> D[Danger Arrives]
    D --> E[Stakes Feel Higher]
```
...
```mermaid id="eq1n5m"
flowchart TD
    A[Create Protagonist Introduction] --> B[1. List Important Traits]
    B --> C[2. Choose the Perfect Scene]
    C --> D[3. Use Interaction and Reaction]
```
...
```mermaid id="g9w9ke"
flowchart TD
    A[Tony Stark Introduction] --> B[Public Recognition]
    B --> C[Brilliant Inventor]
    B --> D[Arrogant and Detached]
    B --> E[Charming but Self-Absorbed]
    E --> F[Character Arc Begins]
```
...
```mermaid id="5z011l"
flowchart LR
    A[Character's Familiar Environment] --> B[Shows Skill]
    A --> C[Shows Status]
    A --> D[Shows Routine]
    A --> E[Shows Relationships]
    A --> F[Shows Flaws]
```
...
```mermaid id="eb7t20"
flowchart TD
    A[Other Characters React] --> B[Reader Learns Reputation]
    A --> C[Reader Sees Conflict]
    A --> D[Reader Understands Social Role]
    A --> E[Reader Judges the Protagonist]
```
...
```mermaid id="o8gchi"
flowchart LR
    A[Ghost Appears] --> B[Expected Reaction: Fear]
    B --> C[Actual Reaction: Practical Advice]
    C --> D[Ghost Feels Humiliated]
    D --> E[Central Conflict Revealed]
```
...
```mermaid id="e646ge"
flowchart TD
    A[Act 1] --> B[Opening Hook]
    B --> C[Introduce Protagonist]
    C --> D[Reveal Desire or Lack]
    D --> E[Suggest Conflict]
    E --> F[Move Toward Inciting Incident]
```
...
```mermaid id="ez9dl2"
flowchart TD
    A[Check Protagonist Introduction] --> B{Is the protagonist doing something?}
    B -->|Yes| C{Does the action reveal a core trait?}
    B -->|No| D[Give them a meaningful action or choice]

    C -->|Yes| E{Is there conflict or tension?}
    C -->|No| F[Revise the action to reveal personality]

    E -->|Yes| G{Is there a memorable detail?}
    E -->|No| H[Add obstacle, pressure, or contradiction]

    G -->|Yes| I{Does this connect to the later arc?}
    G -->|No| J[Add a specific detail readers will remember]

    I -->|Yes| K[Introduction is strong]
    I -->|No| L[Connect the opening trait to the story's main conflict]
```
