...
```mermaid
flowchart TD
    A[Past Event or Hidden Truth] --> B[Character Keeps It Secret]
    B --> C[Character Changes Behavior]
    C --> D[Other Characters Notice Contradictions]
    D --> E[Pressure Increases]
    E --> F[Risk of Exposure]
    F --> G[Secret Revealed]
    G --> H[Consequences Transform the Story]
```
...
```mermaid
flowchart LR
    A[Small Lie] --> B[Temporary Cover-Up]
    B --> C[New Complication]
    C --> D[Bigger Lie]
    D --> E[Someone Becomes Suspicious]
    E --> F[The Character Takes Greater Risks]
    F --> G[The Secret Becomes Dangerous]
    G --> H[Revelation Scene]
```
