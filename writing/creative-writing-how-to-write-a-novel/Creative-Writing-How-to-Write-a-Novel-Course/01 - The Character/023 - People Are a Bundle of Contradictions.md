...
```mermaid
flowchart TD
    A[Character] --> B[Trait 1]
    A --> C[Conflicting Trait 2]

    B --> D[Creates Expectation]
    C --> E[Breaks or Complicates Expectation]

    D --> F[Reader Understands the Character]
    E --> F

    F --> G[Believable Complexity]
    G --> H[Conflict]
    G --> I[Surprise]
    G --> J[Emotional Realism]
```
...
```mermaid
flowchart LR
    A[Bad Cop] --> B[Does Something Heroic]
    C[Good Cop] --> D[Does Something Terrible]

    B --> E[Audience Reconsiders Moral Labels]
    D --> E

    E --> F[Conflict Becomes More Human]
    F --> G[Characters Feel Disturbing and Real]
```
...
```mermaid
flowchart TD
    A[Character Name] --> B[Creates Association]
    B --> C{Matches the Character?}
    C -->|Yes| D[Reinforces Identity]
    C -->|No| E[Creates Contrast]
    E --> F[Humor, Irony, Mystery, or Symbolism]
```
...
```mermaid
flowchart LR
    A[Character's Identity] --> C[Contradiction]
    B[New Environment] --> C

    C --> D[Discomfort]
    D --> E[Conflict]
    E --> F[Adaptation or Collapse]
```
...
```mermaid
flowchart TD
    A[Daniel Plainview] --> B[Surface Want: Power]
    A --> C[Deep Need: Human Connection]
    ...
    F --> J[Rejects Vulnerability]
    I --> K[Secretly Craves Connection]
...
```mermaid
flowchart TD
    A[Pressure Situation] --> B[Admirable Trait Appears]
    B --> C[Reader Trusts the Character]
    C --> D[Contradictory Flaw Appears]
    D --> E[Another Character Pays the Cost]
    E --> F[Character Feels More Complex]
```
