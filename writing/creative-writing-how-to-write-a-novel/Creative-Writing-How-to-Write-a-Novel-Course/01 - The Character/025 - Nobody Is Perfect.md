...
```mermaid
flowchart TD
    A[Character Trait] --> B{Does it affect the plot?}
    B -->|No| C[Minor Trait]
    B -->|Yes| D[Character Flaw]
    D --> E[Creates Conflict]
    E --> F[Forces Growth or Failure]
```
...
```mermaid
flowchart LR
    A[Goal] --> B[External Conflict]
    A --> C[Internal Flaw]
    B --> D[Plot Battle]
    C --> E[Character Battle]
    D --> F[Climax]
    E --> F
    F --> G[Change or Failure]
```
