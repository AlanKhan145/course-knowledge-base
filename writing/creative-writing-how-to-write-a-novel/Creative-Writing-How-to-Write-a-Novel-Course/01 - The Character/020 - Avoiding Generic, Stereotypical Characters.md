...
```mermaid id="specificity-creates-plot"
flowchart TD
    A[Character Detail] --> B[Personal History]
    B --> C[Desire or Wound]
    C --> D[Conflict]
    D --> E[Choice]
    E --> F[Plot Direction]
...
```mermaid id="reader-categorization"
flowchart LR
    A[First Impression] --> B[Category]
    B --> C[Expectation]
    C --> D{Does the Character Confirm or Break It?}
    D --> E[Confirmed Stereotype]
    D --> F[Surprising Specificity]
```
...
```mermaid id="character-specificity-model"
flowchart TD
    A[Role or Type] --> B[Specific Detail]
    B --> C[Contradiction]
    C --> D[Personal Motivation]
    D --> E[Flaw or Wound]
    E --> F[Unique Choices]
    F --> G[Memorable Character]
