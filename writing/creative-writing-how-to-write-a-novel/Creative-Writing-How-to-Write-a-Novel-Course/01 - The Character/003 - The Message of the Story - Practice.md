...
```mermaid
flowchart TD
    A[Plot: What Happens] --> B[Character Choices]
    B --> C[Consequences]
    C --> D[Character Change]
    D --> E[Theme: What It Means]

    A2[External Events] --> B
    B2[Internal Conflict] --> B
```
...
```mermaid
flowchart LR
    A[Character Problem] --> B[Character Desire]
    B --> C[Action]
    C --> D[Conflict]
    D --> E[Consequences]
    E --> F[Real Need Revealed]
    F --> G[Theme]
```
...
```mermaid
flowchart TD
    A[Central Question] --> B[Theme]
    A --> C[Counter-Theme]

    B --> D[Protagonist's Choices]
    C --> E[Antagonist's Choices]

    D --> F[Conflict]
    E --> F

    F --> G[Final Resolution]
    G --> H[Author's Point of View]
```
