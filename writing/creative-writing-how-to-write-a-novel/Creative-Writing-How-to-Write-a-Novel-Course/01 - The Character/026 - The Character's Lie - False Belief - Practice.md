...
```mermaid id="lie01"
flowchart TD
    A[Past Wound] --> B[Emotional Pain]
    B --> C[False Belief / The Lie]
    C --> D[Temporary Protection]
    D --> E[Limited Life]
    E --> F[Story Conflict]
    F --> G[Change or Refusal to Change]
```
...
```mermaid id="lie02"
flowchart LR
    A[Character Believes the Lie] --> B[Plot Event Challenges It]
    B --> C[Character Resists Change]
    C --> D[Consequences Grow]
    D --> E[Lie Becomes Painful]
    E --> F[Moment of Choice]
    F --> G[Accept Truth]
    F --> H[Cling to Lie]
```
...
```mermaid id="lie03"
flowchart TD
    A[Beginning: The Lie Feels Safe] --> B[Inciting Incident Disrupts Life]
    B --> C[Character Tries to Keep Old Belief]
    C --> D[Conflict Exposes the Lie]
    D --> E[Crisis: The Lie No Longer Works]
    E --> F[Choice: Lie or Truth]
    F --> G[Positive Arc: Accepts Truth]
    F --> H[Negative Arc: Clings to Lie]
```
