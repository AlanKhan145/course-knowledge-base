...
```mermaid id="film031a"
flowchart TD
    A[Choose a Film or Novel] --> B[Identify the Protagonist]
    B --> C[Find Their Goal]
    C --> D[Identify the Obstacle]
    D --> E[Discover the Lie]
    E --> F[Find the Need]
    F --> G[Track the Internal Conflict]
    G --> H[Observe the Final Change]
    H --> I[Extract a Craft Lesson]
```
...
```mermaid id="film031b"
flowchart LR
    A[Want] --> B[External Plot]
    C[Need] --> D[Internal Arc]
    E[Lie] --> F[Internal Conflict]
    B --> G[Climax]
    D --> G
    F --> G
    G --> H[Final Choice]
    H --> I[Change or Refusal to Change]
```
