...
```mermaid id="wantneed29a"
flowchart TD
    A[The Character Believes a Lie] --> B[The Lie Creates an Inner Lack]
    B --> C[The Character Chases a Want]
    C --> D[The Plot Tests the Want]
    D --> E[The Character Discovers a Need]
    E --> F{Final Choice}
    F -->|Choose the Want| G[External Victory / Inner Emptiness]
    F -->|Choose the Need| H[Internal Growth / Possible Sacrifice]
    F -->|Gain Both| I[Complete Positive Arc]
    F -->|Lose Both| J[Tragic Arc]
```
...
```mermaid id="wantneed29b"
flowchart LR
    A[Want] --> B[External Goal]
    C[Need] --> D[Internal Truth]
    B --> E[Ending Choice]
    D --> E
    E --> F[What Does the Character Choose?]
    F --> G[Theme Revealed]
```
