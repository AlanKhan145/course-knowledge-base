...
```mermaid
flowchart TD
    A[Writer's Life Question] --> B[Personal Obsession]
    B --> C[Story Idea]
    C --> D[Character Problem]
    D --> E[Character Desire]
    E --> F[Conflict and Choices]
    F --> G[Story Theme]
```
...
```mermaid
flowchart LR
    A[Personal Question] --> B[Emotional Core]
    B --> C[Fictional Character]
    C --> D[External Problem]
    D --> E[Specific Desire]
    E --> F[Story Conflict]
    F --> G[Thematic Ending]
```
