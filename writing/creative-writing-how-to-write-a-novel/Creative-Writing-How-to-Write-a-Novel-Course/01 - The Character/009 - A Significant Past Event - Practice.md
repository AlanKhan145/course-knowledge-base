...
flowchart TD
    ...
    B --> C[Hidden Knot]
    C --> D[Present-Day Behavior]
    D --> E[Conflict in the Story]
    E --> F[Event that forces the character to face the knot]
    F --> G[Character untangles it or remains trapped]
```
...
```mermaid id="qielol"
flowchart LR
    A[Character carries unresolved past] --> B[Present story begins]
    B --> C[New event pressures the wound]
    C --> D[Character resists or avoids it]
    D --> E[Conflict grows]
    E --> F[Character faces the knot]
    F --> G[Transformation or failure]
```
...
```mermaid id="shvuib"
flowchart TD
    A[Past Event] --> B{Does it still affect present behavior?}
    B -->|Yes| C[Strong Knot]
    B -->|No| D[Weak Detail]

    C --> E[Shapes fear, choices, conflict, and arc]
    D --> F[May be interesting but not structurally important]
```
