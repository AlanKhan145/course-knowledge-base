...
```mermaid id="dvzxq3"
flowchart TD
    A[Character's Life] --> B[Problem Appears]
    B --> C[Discomfort or Pressure]
    C --> D[Desire Emerges]
    D --> E[Character Makes a Plan]
    E --> F[Action and Conflict]
    F --> G[Plot Movement]
    G --> H[Change or Consequence]
```
...
```mermaid id="ggzjgz"
flowchart LR
    A[Character Wants Something] --> B[Obstacle Blocks the Way]
    B --> C[Reader Asks a Question]
    C --> D[Will the Character Succeed?]
    D --> E[Reader Keeps Reading]
```
