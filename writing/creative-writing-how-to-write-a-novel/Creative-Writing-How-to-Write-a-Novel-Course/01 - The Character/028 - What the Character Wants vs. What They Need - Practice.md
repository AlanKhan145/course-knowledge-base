...
```mermaid id="wantneed01"
flowchart TD
    A[The Lie] --> B[Character Feels Incomplete]
    B --> C[Character Wants Something External]
    C --> D[Plot Conflict]
    D --> E[Want and Need Collide]
    E --> F[Character Faces the Truth]
    F --> G[Transformation]
```
...
```mermaid id="wantneed02"
flowchart LR
    A[Character's Want] --> C[External Goal]
    B[Character's Need] --> D[Internal Truth]
    C --> E{Conflict}
    D --> E
    E --> F[Choice]
    F --> G[Chase the Want]
...
```mermaid id="rainman01"
flowchart TD
    A[Charlie Wants the Money] --> B[He Takes Raymond from the Institution]
    B --> C[The Brothers Travel Together]
    C --> D[Charlie Begins to Know Raymond]
    D --> E[Charlie Discovers Brotherly Love]
    E --> F[Charlie Realizes Raymond Needs Care]
    F --> G[Charlie Gives Up Control of the Money]
    G --> H[Charlie Accepts the Truth]
```
