...
```mermaid id="bnfvbe"
flowchart TD
    A[Character Action] --> B[Reader observes behavior]
    B --> C[Reader forms judgment]
    C --> D{Is the motivation revealed?}
    D -->|No| E[Reader may misunderstand the action]
    D -->|Yes| F[Reader reinterprets the action]
    F --> G[Character becomes deeper and more believable]
```
...
```mermaid id="dgiedr"
flowchart TD
    A[Layer 1: Superficial Motivation] --> B[Layer 2: Psychological Motivation]
    B --> C[Layer 3: Deep Motivation]
    C --> D[Character Transformation]
```
...
```mermaid id="pbeybl"
mindmap
  root((Marco's Motivation))
    Superficial
      He does not want to sell the house
      He wants to keep control of his life
    Psychological
      He fears losing stability
      He is obsessed with control
      He cannot accept change
    Deep
      He needs to forgive his ex-wife
...
```mermaid id="ozdwhb"
flowchart LR
    A[What the character thinks they want] --> B[Conflict tests them]
    B --> C[Psychological wound is exposed]
    C --> D[Deep motivation becomes visible]
    D --> E[Character changes or refuses to change]
```
...
```mermaid id="cmmdbd"
flowchart TD
    A[Past experience] --> B[Psychological wound]
    B --> C[Core motivation]
    C --> D[Repeated choices]
    D --> E[Major obstacle]
    E --> F[Final decision]
    F --> G[Inevitable character action]
```
