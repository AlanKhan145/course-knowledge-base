...
```mermaid id="habit-character-structure"
flowchart TD
    A[Visible Habit] --> B[Hidden Trait]
    B --> C[Emotional Reason]
    C --> D[Story Pressure]
    D --> E[Change, Break, or Deepen]
...
```mermaid id="habit-arc"
flowchart LR
    A[Habit Introduced] --> B[Habit Repeated]
    B --> C[Habit Reveals Inner Trait]
    C --> D[Plot Challenges the Trait]
    D --> E[Habit Changes or Breaks]
    E --> F[Reader Feels Character Growth]
```
...
```mermaid id="habit-plot-connection"
flowchart TD
    A[Character Habit] --> B[Character Flaw]
    B --> C[Obstacle Becomes Harder]
    C --> D[Conflict Escalates]
    D --> E[Character Must Choose]
    E --> F[Keep the Habit]
    E --> G[Break the Habit]

    F --> H[Failure or Tragedy]
    G --> I[Growth or Transformation]
```
