...
```mermaid id="jhfnyw"
flowchart TD
    A[Character Action] --> B[Superficial Motivation]
    B --> C[Psychological Motivation]
    C --> D[Deep Motivation]
    D --> E[Character Arc]

    B --> B1[What the character says they want]
    C --> C1[Why they behave this way]
    D --> D1[What they truly need but may not know]
```
...
```mermaid id="lsbqis"
flowchart LR
    A[Stated Motivation] --> C[Character Decision]
    B[Real Motivation] --> C
    C --> D{Which motivation wins?}
    D --> E[Character reveals who they are]
```
