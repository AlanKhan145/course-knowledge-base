...
```mermaid
flowchart TD
    A[Dialogue Scene] --> B{Is every answer spoken directly?}
    B -->|Yes| C[Scene may feel too obvious]
    B -->|No| D[Silence creates subtext]

    D --> E[Reader interprets meaning]
    E --> F[Hidden emotion]
    E --> G[Unspoken conflict]
    E --> H[Backstory hint]
    E --> I[Power dynamic]
    E --> J[Character psychology]

    C --> K[Consider replacing one line with silence, pause, or action]
    K --> D
```
...
```mermaid
flowchart LR
    A[Spoken Dialogue] --> B[Surface Meaning]
    A --> C[Unspoken Meaning]

    C --> D[What the character hides]
    C --> E[What the character fears]
    C --> F[What the character wants]
    C --> G[What the other character understands]
    C --> H[What the reader suspects]
```
