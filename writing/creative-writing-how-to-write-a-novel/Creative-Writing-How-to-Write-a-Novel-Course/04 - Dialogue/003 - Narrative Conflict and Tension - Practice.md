...
```mermaid
flowchart TD
    A[Dialogue Scene] --> B[Character A Wants Something]
    A --> C[Character B Wants Something Different]
    B --> D[Conflict]
    C --> D
    D --> E[Narrative Tension]
    E --> F[Reader Curiosity]
    F --> G[Story Movement]
```
...
```mermaid
flowchart TD
    A[Character A Wants the Prize] --> C[Same Goal]
    B[Character B Wants the Prize] --> C
    C --> D[Competition]
    D --> E[Tension]
    E --> F[Possible Betrayal]
    E --> G[Damaged Relationship]
    E --> H[Plot Escalation]
```
...
```mermaid
flowchart TD
    A[Opposing Goals] --> B[Conflict]
    B --> C[Raise the Stakes]
    C --> D[Failure Has a Serious Cost]
    D --> E[Every Word Matters]
    E --> F[High Narrative Tension]
```
...
```mermaid
flowchart TD
    A[Spoken Words] --> B[Surface Meaning]
    A --> C[Hidden Meaning]
    C --> D[Emotional Tension]
    D --> E[Reader Senses a Bigger Problem]

    B --> F[Mayonnaise on a Sandwich]
    C --> G[Anger, Rejection, Resentment]
```
...
```mermaid
flowchart TD
    A[Dialogue Conflict] --> B[Opposing Viewpoints]
    A --> C[Same Goal]
    A --> D[High Stakes]
    A --> E[Refusal / Saying No]

    B --> B1[Light disagreement]
    C --> C1[Competition]
    D --> D1[Danger or serious consequences]
    E --> E1[Blocked desire or hidden resentment]
```
...
```mermaid
flowchart TD
    A[Same Situation: Lottery Win] --> B[Scene 1: Small Conflict]
    A --> C[Scene 2: Bigger Conflict]

    B --> B1[Car vs House Renovation]
    B1 --> B2[Different Priorities]

    C --> C1[Ticket Thrown Away]
    C1 --> C2[Money May Be Lost]
    C2 --> C3[Trust and Relationship Damage]
```
