...
```mermaid
flowchart TD
    A[Scene Context] --> B[Setting]
    A --> C[Historical Period]
    A --> D[Genre]
    A --> E[Social Rules]
    A --> F[Emotional Situation]

    B --> G[Character Reaction]
    C --> G
    D --> G
    E --> G
    F --> G
    ...
    G --> H[Dialogue]
    H --> I[Reader Understands Context Naturally]
```
...
```mermaid
flowchart LR
    A[Context Detail] --> B{Would the character notice it?}
    B -- Yes --> C{Would they mention it now?}
    B -- No --> X[Use narration or cut it]
    C -- Yes --> D[Put it into dialogue]
    C -- No --> Y[Save it for later]
    D --> E[Make it serve the scene]
```
