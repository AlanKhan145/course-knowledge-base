...
```mermaid id="f9k2pa"
flowchart TD
    A[External Situation] --> B[Spoken Dialogue]
    B --> C[Private Reaction]
    C --> D[Interior Monologue]
    D --> E[Hidden Fear, Desire, or Conflict]
    E --> F[Return to Scene Action]
```
...
```mermaid id="h2m8df"
flowchart LR
    A[Backstage] --> B[First Performance]
    B --> C[Minutes Before Announcement]
    C --> D[Inner Panic]
    D --> E[Self-Encouragement]
    E --> F[Comic Imagination]
    F --> G[Walk Onto Stage]
```
