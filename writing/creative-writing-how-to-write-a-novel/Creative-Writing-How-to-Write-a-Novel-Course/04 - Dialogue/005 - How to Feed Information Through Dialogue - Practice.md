...
```mermaid
flowchart TD
    A[Information the Reader Needs] --> B{Should It Be in Dialogue?}

    B -->|No| C[Use Action, Description, or Interior Thought]
    B -->|Yes| D[Give the Speaker a Reason to Say It]

    D --> E[Add Conflict or Emotional Pressure]
    E --> F[Use Subtext and Partial Information]
    F --> G[Let the Other Character React]
    G --> H[Reader Understands Naturally]
...
```mermaid
flowchart LR
    A[Character Wants Something] --> B[Conversation Begins]
    B --> C[Resistance from Other Character]
    C --> D[Conflict Escalates]
    D --> E[Information Slips Out]
    E --> F[Emotional Meaning Changes]
    F --> G[Scene Moves Forward]
```
