...
Bai nay giai thich **CSP** trong nhom **API Security Best Practices** cua module **Learn about APIs**. Sau bai hoc, ban nen biet vai tro cua no va cach no xuat hien trong he thong backend.
...
- Content Security Policy giam rui ro XSS bang cach han che nguon tai nguyen.
- Backend thuong set CSP header cho HTML response.
- Nen bat dau voi report-only truoc khi enforce.
...
**CSP** la mot moc trong lo trinh backend. Hay bien no thanh mot vi du nho, mot checklist debug, hoac mot project mini de kien thuc co cho bam.
