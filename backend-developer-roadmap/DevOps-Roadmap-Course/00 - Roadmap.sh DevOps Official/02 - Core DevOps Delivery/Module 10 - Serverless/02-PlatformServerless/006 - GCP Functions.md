...
Bai nay giai thich **GCP Functions** trong boi canh DevOps/SRE hien dai. Sau bai hoc, ban nen biet no la gi, nam o dau trong workflow van hanh va cach bien no thanh thuc hanh serverless nho.

## 2. Muc tieu hoc tap

- Giai thich duoc GCP Functions bang ngon ngu cua ban.
- Nhan biet no xuat hien o dau trong DevOps workflow thuc te.
- Ap dung vao mot demo, command, config, pipeline hoac runbook nho.

## 3. Khai niem chinh

- GCP Functions giam viec quan ly server nhung them rang buoc ve runtime, timeout va cold start.
...
- Tao mot file cau hinh hoac pseudo-plan cho GCP Functions.
...
- Toi co the giai thich **GCP Functions** trong 1-2 phut.
...
**GCP Functions** la mot moc trong lo trinh DevOps. Hay bien no thanh mot script, mot config, mot dashboard, mot pipeline, hoac mot runbook nho de kien thuc co cho bam.
