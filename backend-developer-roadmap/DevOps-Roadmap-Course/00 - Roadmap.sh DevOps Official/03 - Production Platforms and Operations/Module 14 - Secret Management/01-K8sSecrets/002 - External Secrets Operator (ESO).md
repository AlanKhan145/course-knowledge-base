# 002 - External Secrets Operator (ESO)
...
Bai nay giai thich **External Secrets Operator (ESO)** trong boi canh DevOps/SRE hien dai. Sau bai hoc, ban nen biet no la gi, nam o dau trong workflow van hanh va cach bien no thanh thuc hanh secret management nho.

## 2. Muc tieu hoc tap

- Giai thich duoc External Secrets Operator (ESO) bang ngon ngu cua ban.
- Nhan biet no xuat hien o dau trong DevOps workflow thuc te.
- Ap dung vao mot demo, command, config, pipeline hoac runbook nho.

## 3. Khai niem chinh

- External Secrets Operator (ESO) lien quan den cach luu, ma hoa, cap phat va xoay vong secret.
...
- Toi co the giai thich **External Secrets Operator (ESO)** trong 1-2 phut.
...
**External Secrets Operator (ESO)** la mot moc trong lo trinh DevOps. Hay bien no thanh mot script, mot config, mot dashboard, mot pipeline, hoac mot runbook nho de kien thuc co cho bam.
