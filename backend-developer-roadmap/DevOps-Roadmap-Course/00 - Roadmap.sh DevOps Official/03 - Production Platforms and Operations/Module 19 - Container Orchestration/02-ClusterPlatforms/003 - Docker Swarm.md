...
Bai nay giai thich **Docker Swarm** trong boi canh DevOps/SRE hien dai. Sau bai hoc, ban nen biet no la gi, nam o dau trong workflow van hanh va cach bien no thanh thuc hanh orchestration nho.

## 2. Muc tieu hoc tap

- Giai thich duoc Docker Swarm bang ngon ngu cua ban.
- Nhan biet no xuat hien o dau trong DevOps workflow thuc te.
- Ap dung vao mot demo, command, config, pipeline hoac runbook nho.

## 3. Khai niem chinh

- Docker Swarm giup chay container tren nhieu node voi scheduling, scaling va rollout.
- Can hieu service discovery, health check, resource limit, network va secret.
- Orchestration tang reliability nhung can observability va operational discipline.
...
- Tao mot file cau hinh hoac pseudo-plan cho Docker Swarm.
...
- Toi co the giai thich **Docker Swarm** trong 1-2 phut.
...
**Docker Swarm** la mot moc trong lo trinh DevOps. Hay bien no thanh mot script, mot config, mot dashboard, mot pipeline, hoac mot runbook nho de kien thuc co cho bam.
