...
Bai nay giai thich **Elastic Stack** trong boi canh DevOps/SRE hien dai. Sau bai hoc, ban nen biet no la gi, nam o dau trong workflow van hanh va cach bien no thanh thuc hanh logging nho.

## 2. Muc tieu hoc tap

- Giai thich duoc Elastic Stack bang ngon ngu cua ban.
- Nhan biet no xuat hien o dau trong DevOps workflow thuc te.
- Ap dung vao mot demo, command, config, pipeline hoac runbook nho.

## 3. Khai niem chinh

- Elastic Stack gom Elasticsearch, Logstash, Beats va Kibana cho search/log analytics.
- Can hieu index, mapping, retention, shard va ingest pipeline.
- Logging production can policy ve PII, storage cost va query performance.
...
- Chon 3 tin hieu can theo doi khi dung Elastic Stack.
...
- Toi co the giai thich **Elastic Stack** trong 1-2 phut.
...
**Elastic Stack** la mot moc trong lo trinh DevOps. Hay bien no thanh mot script, mot config, mot dashboard, mot pipeline, hoac mot runbook nho de kien thuc co cho bam.
