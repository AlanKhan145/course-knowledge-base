...
```mermaid
flowchart BT
    A["1. CALORIE<br/>Tổng năng lượng"] --> B["2. MACRONUTRIENT<br/>Protein · Carb · Fat"]
    B --> C["3. MEAL TIMING<br/>Thời gian và số bữa"]
    C --> D["4. FOOD QUALITY<br/>Loại thực phẩm và vi chất"]
    D --> E["5. SUPPLEMENTS<br/>Thực phẩm bổ sung"]

    style A stroke-width:4px
    style B stroke-width:3px
    style C stroke-width:2px
    style D stroke-width:2px
    style E stroke-width:1px
```
...
```mermaid
flowchart TD
    A["Bạn có thích cân và nhập thức ăn không?"] -->|Có| B["Ngân sách calorie/macro"]
    A -->|Không| C["Lịch ăn của bạn có ổn định không?"]
    C -->|Có| D["Thực đơn tương đối cố định"]
    C -->|Không| E["Khuôn mẫu bữa ăn linh hoạt"]
```
...
```mermaid
flowchart LR
    A["Thực hiện kế hoạch"] --> B["Theo dõi 2–3 tuần"]
    B --> C{"Kết quả đúng hướng?"}
    C -->|Có| D["Tiếp tục"]
    C -->|Không| E["Kiểm tra độ tuân thủ"]
    E --> F["Điều chỉnh một biến"]
    F --> A
```
