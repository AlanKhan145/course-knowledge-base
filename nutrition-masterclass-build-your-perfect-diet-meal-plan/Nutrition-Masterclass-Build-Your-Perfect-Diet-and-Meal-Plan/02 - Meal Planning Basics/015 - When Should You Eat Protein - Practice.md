...
![Bữa ăn gồm thịt gà, cơm và rau](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Liat_Portal_for_Foodie_Disorder_-_Grilled_chicken_with_rice_and_vegetables.jpg)
...
```mermaid
flowchart TD
    A["1. Đạt tổng lượng protein mỗi ngày"] --> B["2. Chia protein tương đối đều giữa các bữa"]
    B --> C["3. Ăn một bữa giàu protein quanh buổi tập"]
    C --> D["4. Cân nhắc protein trước khi ngủ"]
    D --> E["5. Chọn loại thực phẩm thuận tiện và dễ tiêu hóa"]
```
...
```mermaid
flowchart LR
    A["Loại protein"] --> D["Tốc độ tiêu hóa và hấp thu"]
    B["Kích thước bữa ăn"] --> D
    C["Chất béo và chất xơ"] --> D
    D --> E["Thời gian amino acid được cung cấp cho cơ thể"]
```
...
![Whey protein](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Whey_protein.jpg)
...
```mermaid
flowchart TD
    A["Bạn có ăn protein trong vài giờ trước tập không?"] -->|Có| B["Không cần vội uống whey ngay lập tức"]
    A -->|Không| C["Ưu tiên một bữa hoặc protein shake sau tập"]
    B --> D["Tiếp tục đạt tổng protein trong ngày"]
    C --> D
```
...
![Cottage cheese — thực phẩm giàu casein](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Cottagecheese200px.jpg)
...
```mermaid
flowchart TD
    A["Bước 1: Xác định mục tiêu protein/ngày"] --> B["Bước 2: Chọn số bữa phù hợp với lịch sống"]
    B --> C["Bước 3: Chia tương đối đều protein giữa các bữa"]
    C --> D["Bước 4: Đặt một bữa protein gần buổi tập"]
    D --> E["Bước 5: Kiểm tra tổng lượng cuối ngày"]
    E --> F["Bước 6: Chỉ bổ sung shake khi thực phẩm thông thường chưa đủ hoặc không tiện"]
```
...
```mermaid
mindmap
  root((Thời điểm ăn protein))
    Tổng lượng mỗi ngày
      Là ưu tiên cao nhất
      Phải phù hợp mục tiêu
    Phân bổ
      Khoảng 3–5 lần ăn
      Tương đối đều giữa các bữa
      Có thể cách nhau khoảng 3–4 giờ
    Loại protein
      Whey tương đối nhanh
      Thực phẩm nguyên dạng trung bình
      Casein tương đối chậm
    Quanh buổi tập
      Không có cửa sổ 30 phút bắt buộc
      Ăn sớm hơn nếu tập khi bụng đói
    Trước khi ngủ
      Không bắt buộc
      Hữu ích khi bữa tối ít protein
      Có thể chọn casein hoặc thực phẩm từ sữa
```
