...
```mermaid
flowchart TD
    A[Chất béo trong thực phẩm] --> B[Chất béo bão hòa]
    A --> C[Chất béo không bão hòa]
    A --> D[Chất béo chuyển hóa]

    C --> E[Không bão hòa đơn - MUFA]
    C --> F[Không bão hòa đa - PUFA]
    ...
    G --> I[ALA]
    G --> J[EPA và DHA]
    H --> K[Linoleic acid - LA]

    B --> L[Nên giới hạn]
    E --> M[Ưu tiên]
    F --> N[Ưu tiên]
    D --> O[Giảm xuống thấp nhất]
```
...
```mermaid
flowchart LR
    A[Muốn lựa chọn chất béo tốt hơn] --> B{Loại thực phẩm?}

    B -->|Cá, hạt, quả bơ, dầu thực vật| C[Ưu tiên với khẩu phần phù hợp]
    B -->|Bơ, thịt mỡ, phô mai, dầu dừa| D[Dùng vừa phải]
    B -->|Dầu hydro hóa một phần| E[Tránh hoặc giảm tối đa]

    C --> F[MUFA và PUFA]
    D --> G[Chủ yếu chất béo bão hòa]
    E --> H[Trans fat công nghiệp]
```
