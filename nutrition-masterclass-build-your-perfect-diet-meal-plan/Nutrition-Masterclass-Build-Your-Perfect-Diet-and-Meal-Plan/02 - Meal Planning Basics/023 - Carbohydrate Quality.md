...
![Các nguồn carbohydrate trong chế độ ăn](https://www.cdc.gov/diabetes/media/images/2024/05/Diabetes-and-carbs-thumbnail.jpg)
...
```mermaid
flowchart LR
    A[Ăn thực phẩm chứa carbohydrate] --> B[Carbohydrate được tiêu hóa]
    B --> C[Glucose đi vào máu]
    C --> D{Tốc độ và mức tăng}
    D -->|Tăng chậm hơn| E[GI thấp]
    D -->|Mức trung bình| F[GI trung bình]
    D -->|Tăng nhanh hơn| G[GI cao]
```
...
![Thực phẩm giàu chất xơ](https://nutritionsource.hsph.harvard.edu/wp-content/uploads/2022/04/shutterstock_675156655-1024x683.jpg)
...
```mermaid
flowchart TD
    A[Thực phẩm chứa carbohydrate] --> B[Chất xơ]
    A --> C[Protein]
    A --> D[Chất béo]
    A --> E[Acid từ chanh hoặc giấm]

    B --> F[Tiêu hóa và hấp thu chậm hơn]
    C --> F
    D --> F
    E --> F

    F --> G[Phản ứng đường huyết của cả bữa ăn thay đổi]
```
...
```mermaid
flowchart TD
    A[Chọn nguồn carbohydrate] --> B{Ít tinh chế?}
    B -->|Có| C{Có chất xơ và vi chất?}
    B -->|Không| D[Kiểm tra khẩu phần và tần suất]

    C -->|Có| E{Ít đường bổ sung?}
    C -->|Không| D

    E -->|Có| F[Nguồn carb ưu tiên]
    E -->|Không| D

    D --> G[Có thể sử dụng có kiểm soát]
```
...
![Mì và ngũ cốc nguyên cám](https://www.cdc.gov/diabetes/media/images/2024/05/half-wheat_16.9_resize.png)
...
```mermaid
flowchart LR
    A[Carbohydrate] --> E[Bữa ăn hoàn chỉnh]
    B[Protein] --> E
    C[Rau và chất xơ] --> E
    D[Chất béo phù hợp] --> E
    E --> F[No lâu hơn]
    E --> G[Dinh dưỡng cân bằng hơn]
    E --> H[Phản ứng đường huyết ổn định hơn]
```
