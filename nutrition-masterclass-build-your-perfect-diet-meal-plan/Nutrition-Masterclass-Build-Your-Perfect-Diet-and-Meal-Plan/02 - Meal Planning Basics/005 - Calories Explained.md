...
```mermaid
flowchart LR
    A[Calorie từ thức ăn và đồ uống] --> B{So sánh với năng lượng tiêu hao}

    B -->|Nạp vào lớn hơn tiêu hao| C[Thặng dư calorie]
    B -->|Nạp vào xấp xỉ tiêu hao| D[Duy trì]
    B -->|Nạp vào nhỏ hơn tiêu hao| E[Thâm hụt calorie]

    C --> F[Tăng năng lượng dự trữ và tăng cân theo thời gian]
    D --> G[Cân nặng trung bình tương đối ổn định]
    E --> H[Sử dụng năng lượng dự trữ và giảm cân theo thời gian]
```
...
```mermaid
flowchart TB
    W[Cân nặng trên cân] --> F[Mỡ cơ thể]
    W --> M[Khối lượng cơ]
    W --> G[Glycogen]
    W --> H[Nước trong cơ thể]
    W --> D[Thức ăn trong hệ tiêu hóa]
    W --> S[Ảnh hưởng của muối và carbohydrate]
```
...
```mermaid
flowchart TB
    T[TDEE<br/>Tổng năng lượng tiêu hao hằng ngày]

    T --> B[BMR hoặc RMR<br/>Duy trì hoạt động sống]
    T --> F[TEF<br/>Tiêu hóa và xử lý thức ăn]
    T --> N[NEAT<br/>Hoạt động ngoài tập luyện]
    T --> E[EAT<br/>Tập luyện có chủ đích]
```
...
```mermaid
flowchart TD
    A[Ăn ở mức calorie ước tính] --> B[Theo dõi 14–21 ngày]
    B --> C{Cân nặng trung bình thay đổi thế nào?}

    C -->|Ổn định| D[Đây có thể gần mức maintenance]
    C -->|Giảm| E[Đang có thâm hụt thực tế]
    C -->|Tăng| F[Đang có thặng dư thực tế]
```
