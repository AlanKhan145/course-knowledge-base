...
```mermaid
flowchart TD
    A[Bắt đầu] --> B{Bạn có tập luyện thường xuyên?}

    B -- Không --> C[0,8–1,2 g/kg/ngày]
    B -- Có --> D{Mục tiêu hiện tại là gì?}

    D -- Duy trì --> E[1,4–2,0 g/kg/ngày]
    D -- Tăng cơ --> F[1,6–2,2 g/kg/ngày]
    D -- Giảm mỡ --> G[1,8–2,4 g/kg/ngày]

    C --> H{Tỷ lệ mỡ cơ thể cao?}
    E --> H
    F --> H
    G --> H

    H -- Không --> I[Tính theo cân nặng hiện tại]
    H -- Có --> J[Tính theo khối lượng không mỡ hoặc cân nặng mục tiêu]

    I --> K[Chia thành 3–5 bữa]
    J --> K
```
