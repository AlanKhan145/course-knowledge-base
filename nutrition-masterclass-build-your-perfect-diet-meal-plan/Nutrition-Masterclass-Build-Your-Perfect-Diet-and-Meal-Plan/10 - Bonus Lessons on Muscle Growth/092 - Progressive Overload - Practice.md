...
![Tập bench press với tạ đòn](https://commons.wikimedia.org/wiki/Special\:FilePath/Bench%20Press.jpg?width=1200)
...
```mermaid
flowchart LR
    A[Tập luyện tạo kích thích] --> B[Mệt mỏi tạm thời]
    B --> C[Dinh dưỡng và phục hồi]
    C --> D[Cơ thể thích nghi]
    D --> E[Mức tập cũ trở nên dễ hơn]
    E --> F[Tăng dần yêu cầu tập luyện]
    F --> A
```
...
```mermaid
flowchart TD
    A[Chọn khoảng 6–8 lần] --> B[Tập với trọng lượng hiện tại]
    B --> C{Đạt 8 lần ở tất cả các hiệp?}
    C -- Chưa --> D[Giữ nguyên tạ và tăng dần số lần lặp]
    D --> B
    C -- Có --> E[Tăng một lượng tạ nhỏ]
    E --> F[Số lần lặp giảm về gần mức 6]
    F --> B
```
...
![Infographic hướng dẫn tập kháng lực của ACSM](https://acsm.org/wp-content/uploads/2026/03/5-things-effective-resistance-training-program-featured-image.png)
