...
![Bữa ăn gồm cá, trứng, bơ và rau](https://commons.wikimedia.org/wiki/Special\:FilePath/Eggs%2Cfried_fish%2Cavocado_and_some_vegetables_on_display.jpg?width=1200)
...
```mermaid
flowchart LR
    A[Ăn carbohydrate] --> B[Đường huyết tăng]
    B --> C[Insulin tăng]
    C --> D[Tăng dự trữ năng lượng]
    D --> E[Tăng mỡ cơ thể]
```
...
![Sơ đồ tác động của insulin đối với hấp thu và chuyển hóa glucose](https://commons.wikimedia.org/wiki/Special\:FilePath/Insulin_glucose_metabolism_ZP.svg)
...
```mermaid
flowchart TD
    A[Chất lượng và lượng carbohydrate] --> B[Ảnh hưởng đường huyết và insulin]
    B --> C[Cảm giác đói, năng lượng và lựa chọn thực phẩm]
    C --> D[Tổng năng lượng ăn vào]
    D --> E[Cân bằng năng lượng dài hạn]
    E --> F[Thay đổi cân nặng và mỡ cơ thể]
```
...
```mermaid
flowchart LR
    A[Nước ngọt và đường bổ sung] --> B[Bánh kẹo và đồ siêu chế biến]
    B --> C[Tinh bột tinh chế]
    C --> D[Điều chỉnh khẩu phần cơm, mì, khoai]
    D --> E[Chỉ giảm sâu hơn khi thật sự cần]
```
