...
![Gia đình chuẩn bị một bữa ăn lành mạnh](https://cdn.who.int/media/images/default-source/imported/behealthy/nutrition.tmb-768v.jpg?sfvrsn=6d120e2b_18)
...
```mermaid
flowchart LR
    A[Một ý tưởng dinh dưỡng] --> B[Người nổi tiếng hoặc mạng xã hội quảng bá]
    B --> C[Chế độ ăn trở thành xu hướng]
    C --> D[Người dùng chia sẻ kết quả ngắn hạn]
    D --> E[Sản phẩm, sách và khóa học xuất hiện]
    E --> F{Kết quả dài hạn có tốt không?}
    F -->|Có bằng chứng| G[Trở thành một lựa chọn hợp lý]
    F -->|Thiếu bằng chứng| H[Dần mất phổ biến]
    H --> A
```
...
```mermaid
flowchart TD
    A[Đánh giá một chế độ ăn] --> B[An toàn]
    A --> C[Hiệu quả]
    A --> D[Đầy đủ dinh dưỡng]
    A --> E[Khả năng duy trì]
    A --> F[Phù hợp cá nhân]

    B --> B1[Không gây thiếu chất hoặc rủi ro sức khỏe]
    C --> C1[Có nghiên cứu trên người hỗ trợ]
    D --> D1[Đủ protein, chất béo, vitamin, khoáng và chất xơ]
    E --> E1[Thực hiện được trong đời sống thực]
    F --> F1[Phù hợp mục tiêu, sở thích, văn hóa và ngân sách]
```
...
![Visual abstract so sánh các mô hình ăn kiêng phổ biến](https://www.bmj.com/content/bmj/369/bmj.m696/F1.large.jpg)
...
![Mô hình Healthy Eating Plate của Harvard](https://nutritionsource.hsph.harvard.edu/wp-content/uploads/2012/09/HEPJan2015.jpg)
...
```mermaid
flowchart TD
    A[Nghe về một chế độ ăn mới] --> B[Xác định nguyên tắc thực sự]
    B --> C[Tìm nguồn nghiên cứu]
    C --> D[Kiểm tra thời gian nghiên cứu]
    D --> E[Đánh giá nguy cơ thiếu chất]
    E --> F[Đánh giá khả năng duy trì]
    F --> G{Phù hợp với bản thân?}
    G -->|Có| H[Thử nghiệm có kiểm soát và theo dõi]
    G -->|Không| I[Chọn phương pháp khác]
    H --> J[Đánh giá lại kết quả]
```
