...
![Mô hình đĩa ăn MyPlate dùng để minh họa các nhóm thực phẩm trong một bữa ăn](https://myplate-prod.azureedge.us/sites/default/files/styles/medium/public/2020-12/icon-circle-myplate--512.png?itok=NZlDvPAC)
...
```mermaid
flowchart LR
    A[Xác định mục tiêu] --> B[Đánh giá sức khỏe và lối sống]
    B --> C[Ước tính nhu cầu dinh dưỡng]
    C --> D[Chọn thực phẩm và cấu trúc bữa ăn]
    D --> E[Thực hiện trong đời sống thực tế]
    E --> F[Theo dõi kết quả]
    F --> G{Kết quả có phù hợp?}
    G -- Có --> H[Tiếp tục và duy trì]
    G -- Chưa --> I[Điều chỉnh khẩu phần hoặc thói quen]
    I --> E
```
...
![Mẫu nhãn Nutrition Facts của FDA](https://www.fda.gov/files/nutrition-facts-label-download-image1.jpg)
...
```mermaid
flowchart TD
    A[Khẩu phần ăn] --> B[Số khẩu phần trong bao bì]
    B --> C[Calories mỗi khẩu phần]
    C --> D[Protein và chất xơ]
    D --> E[Đường bổ sung, natri và chất béo bão hòa]
    E --> F[% Daily Value]
    F --> G[So sánh với sản phẩm tương tự]
```
...
```mermaid
flowchart LR
    A[Chế độ ăn hiệu quả] --> B[Chất lượng thực phẩm]
    A --> C[Phù hợp mục tiêu]
    A --> D[Khả năng duy trì]
    A --> E[Theo dõi và điều chỉnh]

    B --> F[Đủ chất và đa dạng]
    C --> G[Phù hợp năng lượng và protein]
    D --> H[Hợp sở thích, lịch sống và ngân sách]
    E --> I[Thay đổi dựa trên dữ liệu thực tế]
```
