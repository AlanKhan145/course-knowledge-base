...
# 075 — Calcium
[![Calcium and Vitamin D and WS Webinar | Williams Syndrome Association](https://images.openai.com/static-rsc-4/c9Ij-HWNccNJRAEteKTHzQSCKSTXH4Ddf5cRWOfXCNjoPD_Ruf8Wy9rOo6PFFSq-XSXKK_wQRqcf4L_NrnKxUEgFDrQTSTHK7kwRO9WUcL_vlwsX0V0G6Pm9e2hJ1JjqyOnUEwWIQs16Y9eHbb94YKRKkzJMHoKCOLdp6hdSy4E?purpose=inline)](https://www.williams-syndrome.org/resources/webinarpresentation/calcium-and-vitamin-d-and-ws-webinar?utm_source=chatgpt.com)
...
![Các nguồn canxi từ sữa và thực phẩm không chứa sữa](https://commons.wikimedia.org/wiki/Special\:FilePath/Sources%20of%20Calcium.png)
...
```mermaid
flowchart TD
    A[Canxi máu giảm] --> B[Tuyến cận giáp tăng tiết PTH]

    B --> C[Thận giữ lại nhiều canxi hơn]
    B --> D[Tăng hoạt hóa vitamin D]
    B --> E[Tăng huy động canxi từ xương]

    D --> F[Ruột tăng hấp thu canxi]
    C --> G[Canxi máu trở về phạm vi bình thường]
    E --> G
    F --> G
```
...
![Sơ đồ điều hòa cân bằng canxi và phospho](https://commons.wikimedia.org/wiki/Special\:FilePath/Calcium%20and%20phosphorus%20homeostasis.jpg)
...
```mermaid
flowchart LR
    A[Bữa sáng] --> A1[Sữa hoặc sữa đậu nành tăng cường]
    B[Bữa trưa] --> B1[Đậu phụ / cá ăn cả xương]
    C[Bữa phụ] --> C1[Sữa chua]
    D[Bữa tối] --> D1[Cải xoăn / cải thìa / bông cải]
```
...
```mermaid
flowchart TD
    A[Ước tính lượng canxi từ thức ăn] --> B{Đạt nhu cầu hằng ngày?}

    B -- Có --> C[Tiếp tục ưu tiên thực phẩm]
    B -- Không --> D[Điều chỉnh thực đơn và thực phẩm tăng cường]

    D --> E{Vẫn không đạt hoặc có yếu tố nguy cơ?}
    E -- Không --> C
    E -- Có --> F[Trao đổi với bác sĩ hoặc chuyên gia dinh dưỡng]

    F --> G[Chọn liều bù phần còn thiếu]
    G --> H[Kiểm tra bệnh nền và tương tác thuốc]
```
