...
![Cấu trúc ba chiều của oxyhemoglobin](https://upload.wikimedia.org/wikipedia/commons/8/8a/2dn1_oxyhemoglobin_structure.png)
...
```mermaid
flowchart LR
    A[Thực phẩm chứa sắt] --> B{Dạng sắt}
    B -->|Sắt heme| C[Thịt, gia cầm, hải sản]
    B -->|Sắt non-heme| D[Đậu, rau, hạt, ngũ cốc]

    C --> E[Hấp thu tại ruột non]
    D --> E

    E --> F{Trong tế bào ruột}
    F -->|Dự trữ| G[Ferritin]
    F -->|Đưa vào máu| H[Ferroportin]

    H --> I[Transferrin vận chuyển trong máu]
    I --> J[Tủy xương]
    J --> K[Tạo hồng cầu và hemoglobin]
    K --> L[Vận chuyển oxy]

    K --> M[Hồng cầu già]
    M --> N[Đại thực bào thu hồi sắt]
    N --> I
```
...
```mermaid
flowchart LR
    A[Ăn thiếu sắt hoặc mất máu] --> B[Giảm sắt dự trữ]
    B --> C[Ferritin giảm]
    C --> D[Giảm sắt cung cấp cho tủy xương]
    D --> E[Giảm tổng hợp hemoglobin]
    E --> F[Hồng cầu nhỏ và nhạt màu]
    F --> G[Thiếu máu thiếu sắt]
    G --> H[Mệt mỏi, khó thở, giảm hiệu suất]
```
...
![Tiêu bản máu của người bị thiếu máu thiếu sắt](https://upload.wikimedia.org/wikipedia/commons/thumb/f/f2/Iron_deficiency_anemia_blood_film.jpg/960px-Iron_deficiency_anemia_blood_film.jpg)
