...
![Các nguồn carbohydrate từ trái cây, rau củ, ngũ cốc và đậu](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Fruit%2C%20Vegetables%20and%20Grain%20NCI%20Visuals%20Online.jpg?width=1200)
...
```mermaid
flowchart LR
    A[Ăn thực phẩm chứa carbohydrate] --> B[Tiêu hóa thành glucose]
    B --> C[Glucose đi vào máu]
    C --> D[Tuyến tụy tiết insulin]
    D --> E[Tế bào tăng thu nhận glucose]
    E --> F[Sử dụng tạo năng lượng]
    E --> G[Dự trữ thành glycogen]
```
...
![Sơ đồ tác động của insulin lên hấp thu và chuyển hóa glucose](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Insulin_glucose_metabolism.jpg?width=1200)
...
```mermaid
flowchart TD
    A[Nguy cơ kháng insulin] --> B[Dư thừa năng lượng kéo dài]
    A --> C[Ít vận động]
    A --> D[Mỡ nội tạng]
    A --> E[Di truyền và tuổi tác]
    A --> F[Giấc ngủ, bệnh lý hoặc thuốc]
    B --> G[Khả năng kiểm soát glucose suy giảm]
    C --> G
    D --> G
    E --> G
    F --> G
...
```mermaid
flowchart LR
    A[1. Chọn nguồn ít tinh chế] --> B[2. Xác định khẩu phần]
    B --> C[3. Kết hợp protein và rau]
    C --> D[4. Điều chỉnh theo vận động và mục tiêu]
```
