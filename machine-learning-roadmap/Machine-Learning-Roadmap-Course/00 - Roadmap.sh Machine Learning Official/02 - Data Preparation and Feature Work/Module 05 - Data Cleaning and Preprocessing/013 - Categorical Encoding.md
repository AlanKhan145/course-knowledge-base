...
Bai nay giai thich **Categorical Encoding** trong boi canh Machine Learning Engineer Roadmap 2026. Sau bai hoc, ban nen biet topic nay nam o dau trong quy trinh ML, lien quan den data, feature, model, training, evaluation, deployment hoac portfolio nhu the nao.

## 2. Muc tieu hoc tap

- Giai thich duoc Categorical Encoding bang ngon ngu cua ban.
...
- Categorical Encoding is part of the Preprocessing Techniques topic in the Machine Learning roadmap.
- Preprocessing must be learned only from training data to avoid leakage.
- Use pipelines so transformations are repeatable and attached to model training.
...
- Fit transformations on training data only, then apply the fitted transformation to validation/test data.
