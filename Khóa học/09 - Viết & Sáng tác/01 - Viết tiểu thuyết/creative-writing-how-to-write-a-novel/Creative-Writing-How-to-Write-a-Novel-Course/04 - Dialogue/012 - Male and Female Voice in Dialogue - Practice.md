...
```mermaid
flowchart TD
    A[Character Voice] --> B[Personality]
    A --> C[Backstory]
    A --> D[Culture]
    A --> E[Education]
    A --> F[Profession]
    A --> G[Social Class]
    A --> H[Emotional State]
    A --> I[Relationship Dynamic]
    A --> J[Scene Goal]
    A --> K[Gender Expectations]

    K --> L[May influence speech]
    K --> M[Should not define the whole voice]
```
...
```mermaid
flowchart TD
    A[Gender] --> B[Social Expectations]
    B --> C[Upbringing]
    B --> D[Allowed Emotions]
    B --> E[Power and Status]
    B --> F[Politeness Rules]
    B --> G[Risk of Speaking]
