...
```mermaid
flowchart TD
    A[Dialogue] --> B[Spoken Words]
    A --> C[Subtext]

    B --> B1[What characters say]
    C --> C1[What characters mean]
    C --> C2[What characters hide]
    C --> C3[What characters feel]
    C --> C4[What characters want]
```
...
```mermaid
flowchart TD
    A[Surface Topic] --> B[Burnt-Out Light Bulb]
    A --> C[Spoken Request]
    ...
    D --> E[I miss you]
    D --> F[I still need you]
    D --> G[I want an excuse to call]
    D --> H[I cannot say this directly]
```
...
```mermaid
flowchart TD
    A[Spoken Words] --> B[You look so cool]
    A --> C[Surface Compliment]

    D[Nonverbal Signals] --> E[Long Eye Contact]
    D --> F[Awkward Silence]
    D --> G[Tom's Reaction]
    ...
    H --> I[Daisy loves Gatsby]
    H --> J[Tom realizes the betrayal]
```
...
```mermaid
flowchart TD
    A[Surface Conversation] --> B[Old Rug]
    B --> C[It is worn out]
    B --> D[You might trip]
    B --> E[Your knee could be hurt]

    F[Hidden Conversation] --> G[Alex is not married]
    F --> H[Parents are disappointed]
    F --> I[Family pressure]
    F --> J[Fear about the future]
...
```mermaid
flowchart TD
    A[Early Dialogue] --> B[Gandalf speaks about pity]
    B --> C[Frodo does not fully understand]
    C --> D[Idea remains in Frodo's mind]

    D --> E[Later Choice]
    E --> F[Frodo spares Gollum]
    F --> G[The story's ending is affected]

    B --> H[Theme]
    H --> I[Mercy matters]
    H --> J[Judgment has limits]
```
...
```mermaid
flowchart TD
    A[Emotion] --> B[Character Cannot Speak Directly]
    B --> C[Stammering]
    B --> D[Over-Explanation]
    B --> E[Changing the Subject]
    B --> F[Silence]
...
```mermaid
flowchart TD
    A[Character Motivation] --> B[What the Character Wants]
    B --> C[Strategic Speech]

    C --> D[Euphemism]
    C --> E[Politeness]
    C --> F[Indirect Question]
    C --> G[Avoidance]
    C --> H[Flattery]
...
```mermaid
flowchart LR
    A[Surface Meaning] --> B[What is said aloud]
    C[Hidden Meaning] --> D[What is actually meant]

    B --> E[The light bulb is broken]
    D --> F[I miss you]

    B --> G[This rug is dangerous]
    D --> H[You should get married]

    B --> I[You look cool]
    D --> J[I love you]

    B --> K[Would you like tea?]
    D --> L[I am embarrassed]
```
...
```mermaid
flowchart TD
    A[Start with the Real Emotion] --> B[Decide What the Character Wants]
    B --> C[Decide Why They Cannot Say It Directly]
    C --> D[Choose a Surface Topic]
    D --> E[Add Gesture, Silence, or Action]
    E --> F[Let the Reader Infer the Meaning]
```
...
flowchart TD
    ...
    A --> A1[What does A hide?]
    B --> B1[What does B want?]
    C --> C1[What does C know?]
    D --> D1[What does D fear?]
```
