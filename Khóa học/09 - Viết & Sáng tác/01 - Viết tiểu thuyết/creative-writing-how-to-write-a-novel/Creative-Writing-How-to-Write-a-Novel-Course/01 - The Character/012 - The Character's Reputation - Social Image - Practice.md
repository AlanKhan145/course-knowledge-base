...
```mermaid id="h52lqs"
flowchart TD
    A[Character Has a Public Image] --> B[Other Characters Make Assumptions]
    B --> C[The Reader Forms Expectations]
    C --> D[The Character Enters the Story]
    D --> E[Behavior Confirms or Contradicts Reputation]
    E --> F[Conflict Between Image and Truth]
    F --> G[Revelation or Transformation]
```
...
```mermaid id="tuap55"
flowchart LR
    A[Other Characters Talk] --> B[Rumors Spread]
    B --> C[Reader Builds Mental Image]
    C --> D[Expectation Increases]
    D --> E[Character Finally Appears]
    E --> F[Expectation Is Confirmed or Broken]
```
