...
```mermaid id="obstacle-source-map"
flowchart TD
    A[Obstacle] --> B[External Pressure]
    A --> C[Internal Flaw]
...
```mermaid id="obstacle-design-formula"
flowchart LR
    A[Goal] --> B[External Obstacle]
    B --> C[Character Flaw]
    C --> D[Harder Choice]
    D --> E[Greater Tension]
    E --> F[Character Growth or Failure]
```
...
```mermaid id="logical-complication-test"
flowchart TD
    A[New Complication] --> B{Does it increase intensity?}
    B -->|No| C[Remove or revise it]
    B -->|Yes| D{Is it consistent with the story world?}
    D -->|No| E[It feels random]
    D -->|Yes| F{Does it connect to the character?}
    F -->|No| G[It may be only plot noise]
    F -->|Yes| H[Strong complication]
```
