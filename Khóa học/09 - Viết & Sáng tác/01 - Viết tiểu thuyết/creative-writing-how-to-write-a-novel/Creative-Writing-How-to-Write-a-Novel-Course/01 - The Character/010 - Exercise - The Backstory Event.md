...
```mermaid id="gqtmrv"
flowchart TD
    A[Past Event] --> B[Emotional Wound]
    B --> C[Hidden Belief]
    C --> D[Present Behavior]
    D --> E[Reader Question: Why do they act this way?]
    E --> F[Backstory Revealed or Implied]
    F --> G[Reader Understands the Character More Deeply]
```
...
```mermaid id="eznbzl"
flowchart LR
    A[Significant Past Event] --> B[Knot]
    B --> C[Present Fear or Habit]
    C --> D[Plot Conflict]
    D --> E[Pressure Builds]
    E --> F[Untangling Event]
    F --> G[Character Change]
```
...
```mermaid id="ndjgaq"
flowchart TD
    A[What happened?] --> B[How did it wound the character?]
    B --> C[What belief did they form?]
    C --> D[How does it affect them now?]
```
...
```mermaid id="tdfrar"
flowchart TD
    A[Backstory Detail] --> B{Does it affect the main plot or character arc?}
    B -->|Yes| C[Keep it]
    B -->|No| D[Cut or keep only in private notes]

    C --> E{Does the reader need it now?}
    E -->|Yes| F[Reveal it on the page]
    E -->|No| G[Let it appear through behavior first]
```
