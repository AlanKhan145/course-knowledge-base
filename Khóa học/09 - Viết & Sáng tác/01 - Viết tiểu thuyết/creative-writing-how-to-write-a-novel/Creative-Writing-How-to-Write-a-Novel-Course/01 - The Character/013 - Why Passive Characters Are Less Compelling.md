...
```mermaid id="t0lglp"
flowchart TD
    A[External Event Happens] --> B[Character Faces Pressure]
    B --> C[Character Makes a Choice]
    C --> D[Character Takes Action]
    D --> E[Action Creates Consequences]
    E --> F[New Conflict Appears]
    F --> G[Character Must Choose Again]
```
...
```mermaid id="ukf2z9"
flowchart TD
    A[Character Faces an Event] --> B[Choice 1]
    A --> C[Choice 2]
    A --> D[Choice 3]
...
```mermaid id="h0v1lu"
flowchart TD
    A[Goal: Get Home] --> B[Drag the Bicycle]
    A --> C[Leave the Bicycle Behind]
    A --> D[Ask a Stranger for Help]
    A --> E[Call Someone]
    A --> F[Walk Through a Shortcut]

    B --> B1[Slow and exhausting]
    B1 --> B2[Arrives home too late]

    C --> C1[Risk losing the bicycle]
    C1 --> C2[Must explain what happened]

    D --> D1[Stranger may be dangerous]
    D1 --> D2[Creates new relationship or threat]

    E --> E1[Phone battery is low]
    E1 --> E2[Must choose who to call]

    F --> F1[Shortcut is dark or unsafe]
    F1 --> F2[Discovers something unexpected]
```
