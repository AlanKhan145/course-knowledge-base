...
```mermaid id="3uuaow"
flowchart TD
    A[Character Contradiction] --> B[Early Signs]
    B --> C[Repeated Pressure]
    C --> D[Worse Consequences]
    D --> E[Major Choice]
    E --> F[Character Arc Revealed]
```
...
```mermaid id="hk9f5z"
flowchart TD
    A[Character] --> B[Surface Trait]
    A --> C[Hidden Contradiction]

    B --> D[How They Appear to Others]
    C --> E[What They Struggle With Internally]

    D --> F[Reader Expectation]
    E --> G[Unexpected Choice]

    F --> H[Plot Pressure]
    G --> H

    H --> I[Character Must Choose]
    I --> J[Arc Becomes Visible]
```
...
```mermaid id="20uc6f"
flowchart TD
    A[Scene Begins] --> B[Character Faces Pressure]
    B --> C[Trait 1 Pushes One Way]
    B --> D[Trait 2 Pushes Opposite Way]

    C --> E[Internal Conflict]
    D --> E

    E --> F[Character Makes a Choice]
    F --> G[Someone Pays the Cost]
    G --> H[Reader Understands the Arc]
```
...
```mermaid id="67dueb"
flowchart LR
    A[Believes in Peace] --> C[Contradiction]
    B[Acts Cold and Angry] --> C
    C --> D[Forced into War]
    D --> E[Risks Life for Friend]
    E --> F[Reader Sees Hidden Courage]
```
...
```mermaid id="46ncx6"
flowchart TD
    A[Past Failure] --> B[Fear of Responsibility]
    B --> C[Avoids Leadership]
    C --> D[Falls in Love with a Queen]
    D --> E[Love Requires Responsibility]
    E --> F[Must Choose: Safety or Growth]
    F --> G[Accepts Leadership]
```
...
```mermaid id="ymvhz7"
flowchart TD
    A[Opening: Small Contradiction] --> B[First Pressure]
    B --> C[Pattern Repeats]
    C --> D[Midpoint: Bigger Consequence]
    D --> E[Crisis: Contradiction Causes Damage]
    E --> F[Climax: Forced Choice]
    F --> G[Ending: Arc Result]
```
