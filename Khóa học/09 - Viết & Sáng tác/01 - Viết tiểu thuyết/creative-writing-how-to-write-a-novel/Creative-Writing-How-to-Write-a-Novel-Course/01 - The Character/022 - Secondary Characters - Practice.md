...
```mermaid
flowchart TD
    A[Secondary Character] --> B[Reveals the Protagonist]
    A --> C[Creates Conflict]
    A --> D[Supports or Blocks the Goal]
    A --> E[Adds Emotional Texture]
    A --> F[Expands the Story World]
...
```mermaid
flowchart LR
    A[Ghost Character] --> B[Past Wound]
    B --> C[Emotional Pressure]
    C --> D[Protagonist Reflects]
    D --> E[Story Begins or Changes Direction]
```
...
```mermaid
flowchart TD
    A[Teacher] --> B[Sees Protagonist's Weakness]
    B --> C[Creates Challenge]
    C --> D[Protagonist Resists]
    D --> E[Frustration]
    E --> F[Growth]
    F --> G[Transformation]
```
...
```mermaid
flowchart LR
    A[Protagonist Tries to Change] --> B[Counterweight Appears]
    B --> C[Pulls Protagonist Back]
    C --> D[Old Trap or Old Identity]
    D --> E{Will the Protagonist Resist?}
    E -->|Yes| F[Growth Becomes Stronger]
    E -->|No| G[Regression or Crisis]
```
...
```mermaid
flowchart TD
    A[Trust Is Built] --> B[Hidden Motive Exists]
    B --> C[Pressure Increases]
    C --> D[Betrayal or Near-Betrayal]
    D --> E[Story Reversal]
    E --> F{Can the Traitor Repent?}
    F -->|Yes| G[Redemption Arc]
    F -->|No| H[Deeper Conflict]
```
...
```mermaid
flowchart LR
    A[Heavy Dramatic Moment] --> B[Comic Relief Appears]
    B --> C[Humor or Irony]
    C --> D[Emotional Release]
    D --> E[Reader Re-engages]
    E --> F[Drama Feels Sharper by Contrast]
```
...
```mermaid
flowchart TD
    A[Protagonist] --> B[Main Desire]
    A --> C[Main Conflict]
    A --> D[Main Transformation]

    E[Secondary Character] --> F[Personal Desire]
    E --> G[Narrative Function]
    E --> H[Effect on Protagonist]

    F --> I[Feels Real]
    G --> J[Serves the Story]
    H --> K[Reveals the Hero]
```
