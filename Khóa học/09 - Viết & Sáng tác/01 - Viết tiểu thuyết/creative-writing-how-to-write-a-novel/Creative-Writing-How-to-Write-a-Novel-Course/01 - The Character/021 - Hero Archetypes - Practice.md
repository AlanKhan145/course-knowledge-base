...
```mermaid id="hero-archetypes-main"
flowchart TD
    A[Hero Archetypes] --> B[The King]
    A --> C[The Warrior]
    A --> D[The Mage]
    A --> E[The Lover]

    B --> B1[Leadership, justice, responsibility]
    C --> C1[Honor, sacrifice, protection]
    D --> D1[Wisdom, knowledge, mystery]
    E --> E1[Love, passion, harmony]
```
...
```mermaid id="hero-subtypes"
flowchart TD
    A[Hero Subtypes] --> B[The Common Man]
    A --> C[The Anti-Hero]
    A --> D[The Misfit]
    A --> E[The Chosen One / Prodigy]
    A --> F[The Survivor]
    A --> G[The Rebel]
```
...
```mermaid id="archetype-combination-map"
flowchart LR
    A[Base Archetype] --> C[Complete Hero Design]
    B[Hero Subtype] --> C
    D[Specific Flaw] --> C
    E[Personal History] --> C
    F[Unique Desire] --> C

    A1[King / Warrior / Mage / Lover] --> A
    B1[Common Man / Anti-Hero / Misfit / Chosen One / Survivor / Rebel] --> B
```
...
```mermaid id="reader-expectation-subversion"
flowchart TD
    A[Archetype] --> B[Reader Expectation]
    B --> C{Writer's Choice}
    C --> D[Meet the Expectation]
    C --> E[Subvert the Expectation]
    D --> F[Genre Satisfaction]
    E --> G[Surprise and Freshness]
    F --> H[Memorable Hero]
    G --> H
```
