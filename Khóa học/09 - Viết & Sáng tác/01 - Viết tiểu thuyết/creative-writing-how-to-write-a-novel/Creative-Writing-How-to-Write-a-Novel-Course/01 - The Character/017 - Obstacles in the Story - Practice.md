...
```mermaid
flowchart TD
    A[Story Conflict] --> B[Inner Conflict]
    A --> C[Social Conflict]
    A --> D[Situational Conflict]
    A --> E[Relationship Conflict]

    B --> B1[Character vs Self]
    C --> C1[Character vs Group or System]
    D --> D1[Character vs Nature or Survival Situation]
    E --> E1[Character vs Antagonist or Rival]
```
...
```mermaid
flowchart LR
    A[Goal Established] --> B[Obstacle 1: Initial Difficulty]
    B --> C[Obstacle 2: Greater Pressure]
    C --> D[Obstacle 3: Personal Weakness Exposed]
    D --> E[Obstacle 4: Major Sacrifice or Choice]
    E --> F[Climax]
```
...
```mermaid
flowchart TD
    A[Protagonist] --> B[Goal]
    A --> C[Weakness or Fear]
    B --> D[External Obstacle]
    C --> E[Internal Obstacle]
    D --> F[Conflict]
    E --> F
    F --> G[Choice]
    G --> H[Growth or Failure]
```
