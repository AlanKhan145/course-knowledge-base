...
```mermaid
flowchart LR
    A[Act 2A: Reactive Phase] --> B[Midpoint Revelation]
    B --> C[Act 2B: Proactive Phase]
    C --> D[Second Turning Point]

    A1[Confusion] --> A
    A2[Mistakes] --> A
    A3[False assumptions] --> A

    C --> C1[New plan]
    C --> C2[Greater action]
    C --> C3[Renewed setbacks]
    C --> C4[Inner conflict]

    B:::midpoint
    D:::turning

    classDef midpoint fill:#ffe6a7,stroke:#d97706,stroke-width:2px,color:#000;
...
```mermaid
flowchart TD
    A[Second Half of Act 2] --> B[1. Birth of a New Self]
    A --> C[2. Old Self vs New Self]
    A --> D[3. Repeated Scene, New Choice]
    A --> E[4. Second Critical Point]

    B --> B1[The protagonist applies what they learned]
    C --> C1[They are still attached to old desires]
    D --> D1[The same situation returns, but they respond differently]
    E --> E1[The antagonist's danger becomes clear again]
```
...
```mermaid
flowchart TD
    A[Earlier Scene] --> B[Old Choice]
    B --> C[Shows Old Self]

    D[Later Scene] --> E[New Choice]
    E --> F[Shows Transformation]
...
```mermaid
flowchart LR
    A[Midpoint] --> B[Protagonist gains new energy]
    B --> C[Victories and progress]
    C --> D[Second Critical Point]
    D --> E[Danger returns stronger]
    E --> F[Renewed determination]
    F --> G[Second Turning Point]
```
...
```mermaid
flowchart TD
    A[New Plan] --> B[Small Victory]
    B --> C[Unexpected Setback]
    C --> D[Adaptation]
    D --> E[Bigger Victory]
    E --> F[Greater Cost]
    F --> G[Second Critical Point]
    G --> H[Push Toward Second Turning Point]
```
...
```mermaid
flowchart TD
    A[Midpoint Revelation] --> B[The protagonist forms a new plan]
    B --> C[They act with more purpose]
    C --> D[They achieve partial success]
    D --> E[Old self creates inner resistance]
    E --> F[A repeated scene proves change]
    F --> G[The antagonist's danger returns]
    G --> H[The protagonist suffers another loss]
    H --> I[New strength and urgency]
    I --> J[Second Turning Point]
```
