...
```mermaid
flowchart TD
    A[Protagonist Begins with the Truth] --> B[World Represents the Lie]
    B --> C[Conflict Tests the Protagonist]
    C --> D[Protagonist Acts on the Truth]
    D --> E[World or Other Characters Begin to Change]
    E --> F[Truth Is Proven Through Action]
```
...
flowchart LR
    ...
    B --> C[Learns the Truth]
    C --> D[Changes Internally]
```
...
```mermaid
flowchart LR
    A[Already Believes the Truth] --> B[Faces a Corrupt or Opposing World]
    B --> C[Acts on the Truth]
    C --> D[Changes the World Around Them]
```
...
```mermaid
flowchart TD
    A[The Truth] --> C[Protagonist]
    B[The Lie] --> D[World / Society / Villain / System]
    C --> E[Conflict]
    D --> E
    E --> F[Truth Is Tested]
    F --> G[Truth Changes the World]
```
...
```mermaid
flowchart LR
    A[Same Core Protagonist] --> B[New Case or Conflict]
    B --> C[Truth Is Tested]
    C --> D[World or Supporting Character Changes]
    D --> E[Protagonist Remains Fundamentally the Same]
```
...
```mermaid
flowchart TD
    A[Act 1] --> B[Introduce Protagonist's Truth]
    A --> C[Show the World's Lie]
    B --> D[Create Moral Conflict]
    C --> D
```
...
```mermaid
flowchart TD
    A[Act 2] --> B[External Pressure Increases]
    B --> C[Truth Becomes Harder to Defend]
    C --> D[Protagonist Takes Action]
    D --> E[Others Begin to Change]
```
...
```mermaid
flowchart TD
    A[Act 3] --> B[Final Test of the Truth]
    B --> C[Protagonist Refuses the Lie]
    C --> D[Climactic Action]
    D --> E[World or People Are Transformed]
```
...
```mermaid
flowchart TD
    A[Protagonist Holds the Truth] --> B[World Believes the Lie]
    B --> C[Conflict Challenges the Truth]
    C --> D[Protagonist Acts Under Pressure]
    D --> E[Truth Is Demonstrated]
    E --> F[World / Characters Change]
    F --> G[Protagonist Remains Steady]
```
