...
```mermaid id="act2-structure-010"
flowchart LR
    A[Act 1: Setup] --> B[First Turning Point]
    B --> C[Act 2 Begins]
    C --> D[First Half of Act 2]
    D --> E[Midpoint]
    E --> F[Second Half of Act 2]
    ...
    classDef act2 fill:#dcedc8,stroke:#558b2f,stroke-width:2px;
    classDef midpoint fill:#ffe0b2,stroke:#d35400,stroke-width:2px;
```
...
```mermaid id="act2-three-parts-010"
flowchart TD
    A[Act 2: Confrontation] --> B[First Half of Act 2]
    A --> C[Central Part / Midpoint]
    A --> D[Second Half of Act 2]

    B --> E[Confusion, adaptation, first tests]
    C --> F[Major revelation or shift]
    D --> G[Escalation, consequences, deeper pressure]
```
...
```mermaid id="reaction-action-010"
flowchart LR
    A[First Turning Point] --> B[Protagonist Reacts]
    B --> C[Old Methods Fail]
    C --> D[They Receive Tools or Guidance]
    D --> E[They Attempt New Actions]
    E --> F[They Begin Driving the Plot]
```
...
```mermaid id="pain-types-010"
flowchart TD
    A[Act 2 Tests] --> B[Physical Pain]
    A --> C[Psychological Pain]

    B --> D[Training]
    B --> E[Survival]
    B --> F[New Skills]

    C --> G[Letting Go of the Lie]
    C --> H[Facing Fear]
    C --> I[Accepting a New Identity]
```
...
```mermaid id="want-need-010"
flowchart TD
    A[Protagonist Enters Act 2] --> B[They Chase What They Want]
    B --> C[They Use Old Habits]
    C --> D[They Fail or Create New Problems]
    D --> E[They Receive Lessons and Tools]
    E --> F[They Glimpse What They Need]
    F --> G[Inner Conflict Grows]
```
