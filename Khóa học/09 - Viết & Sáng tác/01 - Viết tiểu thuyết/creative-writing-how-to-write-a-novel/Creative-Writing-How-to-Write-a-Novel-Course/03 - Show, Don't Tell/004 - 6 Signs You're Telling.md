...
```mermaid
flowchart TD
    A[Signs You Are Telling] --> B[1. Giving conclusions]
    A --> C[2. Using vague or abstract descriptions]
    A --> D[3. Summarizing important moments]
    A --> E[4. Starting with unnecessary prologue]
    A --> F[5. Naming emotions and sensations directly]
    A --> G[6. Relying on adverbs]

    B --> B1[Reader is told what to think]
    C --> C1[Reader cannot visualize the scene]
    D --> D1[Reader receives information instead of experience]
    E --> E1[Story explains before creating curiosity]
    F --> F1[Emotion is named but not felt]
    G --> G1[Dialogue or action is explained instead of dramatized]
```
...
```mermaid
flowchart TD
    A[Sentence in your draft] --> B{Does it name an emotion, conclusion, or abstract idea?}

    B -->|Yes| C[Possible telling]
    B -->|No| D[May already be showing]

    C --> E{Can the reader see, hear, or feel the moment?}
    E -->|No| F[Rewrite with concrete details]
    E -->|Yes| G[Keep if it works]

    F --> H[Add action]
    F --> I[Add body language]
    F --> J[Add dialogue]
    F --> K[Add sensory detail]
```
