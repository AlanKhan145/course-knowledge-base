...
```mermaid
flowchart TD
    A[Choose One Emotion] --> B[Choose a Scene Situation]
    B --> C[Choose a Technique]
    C --> D[Show Physical Reactions]
    C --> E[Use Dialogue or Silence]
    C --> F[Use Objects and Setting]
    D --> G[Reader Infers Emotion]
    E --> G
    F --> G
```
...
```mermaid
flowchart LR
    A[Big Emotion] --> B[Small Action]
    B --> C[Hidden Pain]
    C --> D[Stronger Reader Response]
```
...
flowchart TD
    ...
    C --> D[Physical Reaction]
    D --> E[Object or Setting Detail]
    E --> F[Dialogue or Silence]
    F --> G[Final Image]
```
...
```mermaid
flowchart TD
    A[What Character Feels] --> B[What Character Cannot Say]
    B --> C[What Character Actually Says]
    C --> D[Subtext]
    D --> E[Reader Feels the Emotion]
```
