...
```mermaid
flowchart TD
    A[Writer wants to express an idea] --> B{Use telling or showing?}

    B --> C[Telling]
    B --> D[Showing]

    C --> C1[State the conclusion directly]
    C1 --> C2[Mary is angry]
    C2 --> C3[Reader receives information]

    D --> D1[Use action, dialogue, body language, and detail]
    D1 --> D2[Mary slams the door and screams]
    D2 --> D3[Reader infers emotion]
    D3 --> D4[Reader becomes engaged]
```
...
```mermaid
flowchart TD
    A[Scene or information] --> B{Is this moment emotionally or dramatically important?}

    B -->|Yes| C[Show it]
    C --> C1[Use action]
    C --> C2[Use dialogue]
    C --> C3[Use body language]
    C --> C4[Use sensory detail]
