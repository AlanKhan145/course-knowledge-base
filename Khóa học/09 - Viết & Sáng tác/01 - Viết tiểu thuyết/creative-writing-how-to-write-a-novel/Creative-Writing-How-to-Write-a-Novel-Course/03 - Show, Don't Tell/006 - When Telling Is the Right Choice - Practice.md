...
```mermaid id="b51xov"
flowchart TD
    A[Should I show or tell?] --> B{Is the moment emotionally or dramatically important?}

    B -->|Yes| C[Show it]
    C --> C1[Inciting incident]
    C --> C2[Turning points]
    C --> C3[Climax]
    C --> C4[Major confrontation]
    C --> C5[Important decision]

    B -->|No| D[Tell it]
    D --> D1[Transitions]
    D --> D2[Minor logistics]
    D --> D3[Repeated information]
    D --> D4[Time jumps]
    D --> D5[First draft placeholders]
```
...
```mermaid id="j4mgbr"
flowchart TD
    A[Scene or information] --> B{Does this moment change the story?}
    ...
    B -->|No| D{Does the reader need to know it?}

    D -->|Yes| E[Tell it briefly]
    E --> E1[Summarize]
    E --> E2[Transition]
    E --> E3[Compress time]
