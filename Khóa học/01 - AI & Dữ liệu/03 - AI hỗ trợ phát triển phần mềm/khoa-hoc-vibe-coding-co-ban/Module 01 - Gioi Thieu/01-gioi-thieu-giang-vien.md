...
```mermaid
flowchart LR
    A["Hiểu ý tưởng"] --> B["Viết prompt rõ ràng"]
    B --> C["Để AI tạo bản đầu tiên"]
    C --> D["Kiểm tra kết quả"]
    D --> E["Tinh chỉnh và sửa lỗi"]
    E --> F["Hoàn thiện sản phẩm"]
```
...
```mermaid
flowchart TD
    A["Lý thuyết vừa đủ"] --> B["Ví dụ minh họa"]
    B --> C["Thực hành trực tiếp"]
    C --> D["Kiểm tra kết quả"]
    D --> E["Sản phẩm hoàn chỉnh cuối bài"]
```
