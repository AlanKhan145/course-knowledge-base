...
Bai nay giai thich **SBOMs** trong boi canh DevSecOps hien dai. Sau bai hoc, ban nen biet topic nay giam risk nao, nam o dau trong lifecycle va co the ap dung vao code, pipeline, cloud, runtime hay incident response.

## 2. Muc tieu hoc tap

- Giai thich duoc SBOMs bang ngon ngu cua ban.
...
- SBOM liet ke thanh phan phan mem va version trong ung dung.
- SBOM giup tra loi nhanh app nao bi anh huong khi co CVE moi.
- SBOM nen duoc sinh trong build va gan voi artifact release.

## 4. Vi du / Demo

```text
Artifact: api:v1.2.3
SBOM: package name, version, license, supplier, hash
Use: answer whether a new CVE affects this release
```
...
- Toi co the giai thich **SBOMs** trong 1-2 phut.
...
**SBOMs** la mot moc trong lo trinh DevSecOps. Hay bien no thanh mot checklist, script, config, scan, detection rule, dashboard hoac runbook de kien thuc co cho bam.
