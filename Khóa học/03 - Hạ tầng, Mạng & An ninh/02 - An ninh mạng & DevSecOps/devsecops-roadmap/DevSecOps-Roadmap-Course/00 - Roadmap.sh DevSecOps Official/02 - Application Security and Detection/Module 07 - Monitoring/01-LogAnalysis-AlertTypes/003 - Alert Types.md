...
Bai nay giai thich **Alert Types** trong boi canh DevSecOps hien dai. Sau bai hoc, ban nen biet topic nay giam risk nao, nam o dau trong lifecycle va co the ap dung vao code, pipeline, cloud, runtime hay incident response.

## 2. Muc tieu hoc tap

- Giai thich duoc Alert Types bang ngon ngu cua ban.
...
- Security alert co the den tu login spike, privilege escalation, suspicious API, malware, exfiltration hoac policy violation.
- Moi alert can co threshold, context, impact va runbook.
- Qua nhieu false positive se lam team bo qua tin hieu that.
...
- Toi co the giai thich **Alert Types** trong 1-2 phut.
...
**Alert Types** la mot moc trong lo trinh DevSecOps. Hay bien no thanh mot checklist, script, config, scan, detection rule, dashboard hoac runbook de kien thuc co cho bam.
