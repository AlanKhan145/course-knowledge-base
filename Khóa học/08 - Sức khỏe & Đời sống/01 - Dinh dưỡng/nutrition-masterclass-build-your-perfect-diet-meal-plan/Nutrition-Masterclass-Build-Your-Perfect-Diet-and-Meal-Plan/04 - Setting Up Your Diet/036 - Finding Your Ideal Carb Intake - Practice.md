...
![Các nguồn thực phẩm giàu carbohydrate và thực phẩm toàn phần](https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Good_Food_Display_-_NCI_Visuals_Online.jpg/960px-Good_Food_Display_-_NCI_Visuals_Online.jpg)
...
![Sơ đồ chuyển hóa và dự trữ glucose](https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Glucose_metabolism.svg/960px-Glucose_metabolism.svg.png)
...
flowchart LR
    ...
    B --> C[Glucose trong máu]
    C --> D[Năng lượng sử dụng ngay]
    C --> E[Glycogen trong gan]
    C --> F[Glycogen trong cơ]
    F --> G[Năng lượng cho buổi tập]
    E --> H[Hỗ trợ duy trì đường huyết]
```
...
```mermaid
flowchart TD
    A[Hôm nay có tập không?] -->|Có| B[Giữ carbohydrate ở mức mục tiêu]
    A -->|Không| C{Đang giảm mỡ?}
    C -->|Không| D[Giữ gần bằng ngày tập]
    C -->|Có| E[Giảm nhẹ 10–20%]
    E --> F[Giữ protein ổn định]
    F --> G[Theo dõi sức mạnh buổi tập kế tiếp]
```
...
```mermaid
flowchart TD
    A[Xác định cân nặng] --> B[Phân loại buổi tập]
    B --> C[Chọn khoảng g/kg hoặc g/lb]
    C --> D[Tính số gram carbohydrate]
    D --> E[Kiểm tra tổng calorie]
    E --> F[Áp dụng trong 10–14 ngày]
    F --> G[Đánh giá sức mạnh, đói, tiêu hóa và cân nặng]
    G --> H[Điều chỉnh 20–40 g nếu cần]
```
...
![Bữa yến mạch và chuối](https://upload.wikimedia.org/wikipedia/commons/thumb/0/04/Banana_oatmeal.jpg/960px-Banana_oatmeal.jpg)
