...
![Đong bột protein để pha đồ uống](https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/User_measures_a_scoop_of_protein_powder_from_a_container_in_a_kitchen.jpg/960px-User_measures_a_scoop_of_protein_powder_from_a_container_in_a_kitchen.jpg)
...
```mermaid
flowchart TD
    A[Muốn tăng cơ hoặc phục hồi tốt hơn] --> B{Protein từ thức ăn đã đủ chưa?}
    B -->|Đã đủ| C[Không bắt buộc dùng protein powder]
    B -->|Chưa đủ| D[Tính lượng protein còn thiếu]
    D --> E[Chọn loại phù hợp với tiêu hóa, ngân sách và chế độ ăn]
    E --> F[Dùng 1 khẩu phần để bù phần thiếu]
```
...
![Forest plot về protein timing sau khi điều chỉnh tổng lượng protein](https://media.springernature.com/full/springer-static/image/art%3A10.1186%2F1550-2783-10-53/MediaObjects/12970_2013_Article_446_Fig3_HTML.jpg)
...
![Các nguồn protein từ thực phẩm](https://upload.wikimedia.org/wikipedia/commons/e/e8/Protein-rich_Foods.jpg)
...
```mermaid
flowchart LR
    A[Chọn chất lỏng] --> B{Mục tiêu chính}
    B -->|Giảm calorie| C[Nước hoặc sữa hạt không đường]
    B -->|Tăng protein| D[Sữa bò hoặc sữa đậu nành]
    B -->|Tăng cân| E[Sữa nguyên kem + chuối + yến mạch]
    B -->|Không dung nạp lactose| F[Nước, sữa không lactose hoặc sữa thực vật]
```
