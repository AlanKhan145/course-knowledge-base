...
![Tập luyện sức mạnh trong giai đoạn giảm mỡ](https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Woman_lifting_dumbbells_in_a_modern_gym_during_a_workout_session_focused_on_strength_training_and_fitness.jpg/960px-Woman_lifting_dumbbells_in_a_modern_gym_during_a_workout_session_focused_on_strength_training_and_fitness.jpg)
...
```mermaid
flowchart TD
    A[Năng lượng từ thực phẩm] --> B{So sánh với tổng năng lượng tiêu hao}
    B -->|Nạp vào lớn hơn tiêu hao| C[Thặng dư năng lượng]
    C --> D[Cân nặng có xu hướng tăng]

    B -->|Nạp vào xấp xỉ tiêu hao| E[Duy trì năng lượng]
    E --> F[Cân nặng tương đối ổn định]

    B -->|Nạp vào thấp hơn tiêu hao| G[Thâm hụt năng lượng]
    G --> H[Cơ thể sử dụng năng lượng dự trữ]
    H --> I[Cân nặng và mỡ có xu hướng giảm]
```
...
```mermaid
flowchart LR
    A[Giảm mỡ hiệu quả] --> B[Thâm hụt năng lượng vừa phải]
    A --> C[Protein và chế độ ăn đủ chất]
    A --> D[Tập kháng lực đều đặn]

    B --> E[Sử dụng mỡ dự trữ]
    C --> F[No lâu và hỗ trợ duy trì cơ]
    D --> G[Duy trì sức mạnh và tín hiệu cơ bắp]
```
...
![Minh họa các nhóm thực phẩm trong một chế độ ăn cân bằng](https://upload.wikimedia.org/wikipedia/commons/thumb/7/7b/Eatwell_Plate.png/960px-Eatwell_Plate.png)
...
```mermaid
flowchart LR
    A["Protein 1,2 g/kg/ngày"] --> B["Khối lượng nạc: +0,1 kg"]
    A --> C["Khối lượng mỡ: −3,5 kg"]

    D["Protein 2,4 g/kg/ngày"] --> E["Khối lượng nạc: +1,2 kg"]
    D --> F["Khối lượng mỡ: −4,8 kg"]
```
...
```mermaid
flowchart TD
    A[1. Thâm hụt năng lượng] --> D[Giảm mỡ]
    B[2. Protein và chế độ ăn đủ chất] --> E[Duy trì cơ và kiểm soát cơn đói]
    C[3. Tập kháng lực] --> F[Duy trì sức mạnh và khối lượng cơ]

    D --> G[Kết quả giảm mỡ bền vững]
    E --> G
    F --> G
```
