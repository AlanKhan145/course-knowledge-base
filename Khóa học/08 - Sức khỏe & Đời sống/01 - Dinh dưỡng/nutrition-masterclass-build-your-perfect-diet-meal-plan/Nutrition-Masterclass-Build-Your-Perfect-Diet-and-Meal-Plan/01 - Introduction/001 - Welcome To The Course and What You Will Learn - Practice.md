...
```mermaid
flowchart LR
    A[Hiểu nguyên lý] --> B[Thu thập dữ liệu cá nhân]
    B --> C[Đặt mục tiêu]
    C --> D[Tính calorie]
    D --> E[Phân bổ macronutrient]
    E --> F[Chọn thực phẩm và số bữa]
    F --> G[Theo dõi kết quả]
    G --> H[Điều chỉnh thực đơn]
    H --> G
```
...
```mermaid
flowchart TD
    A[Khóa học dinh dưỡng] --> B[Phần 1: Meal Planning]
    A --> C[Phần 2: Healthy Dieting]

    B --> B1[Calorie]
    B --> B2[Protein, carbohydrate và chất béo]
    B --> B3[Chất lượng thực phẩm]
    B --> B4[Meal timing]
    B --> B5[Bữa ăn trước và sau tập]
    B --> B6[Giảm mỡ và tăng cơ]
    B --> B7[Thực phẩm bổ sung]

    C --> C1[Vitamin và khoáng chất]
    C --> C2[Nước và hydrat hóa]
    C --> C3[Phá bỏ lầm tưởng]
    C --> C4[Phân tích các chế độ ăn]
    C --> C5[Sức khỏe và năng lượng]
    C --> C6[Kỹ năng tự nghiên cứu]
```
...
```mermaid
flowchart BT
    A[1. Cân bằng calorie và tính nhất quán]
    B[2. Macronutrient: protein, carbohydrate, chất béo]
    C[3. Chất lượng thực phẩm, chất xơ và vi chất]
    D[4. Số bữa và thời điểm ăn]
    E[5. Thực phẩm bổ sung]
...
```mermaid
flowchart LR
    A[Nền tảng] --> B[Thiết kế thực đơn]
    B --> C[Theo dõi và điều chỉnh]
    C --> D[Nội dung nâng cao]
    D --> E[Tự nghiên cứu và duy trì lâu dài]
```
