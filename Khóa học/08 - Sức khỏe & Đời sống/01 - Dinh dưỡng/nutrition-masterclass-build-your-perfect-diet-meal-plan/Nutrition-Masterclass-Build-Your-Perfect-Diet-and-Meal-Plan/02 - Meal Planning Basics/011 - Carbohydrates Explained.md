...
![Các nguồn thực phẩm chứa carbohydrate](https://commons.wikimedia.org/wiki/Special\:FilePath/Fruit%2C_Vegetables_and_Grain_NCI_Visuals_Online.jpg)
...
```mermaid
flowchart LR
    A[Thực phẩm chứa carbohydrate] --> B[Tiêu hóa]
    B --> C[Đường đơn: glucose]
    C --> D[Đi vào máu]
    D --> E[Dùng ngay để tạo năng lượng]
    D --> F[Tích trữ thành glycogen]
    D --> G[Tham gia các con đường chuyển hóa khác]
    
    F --> H[Cơ xương]
    F --> I[Gan]
    
    H --> J[Nhiên liệu cho vận động]
    I --> K[Hỗ trợ duy trì glucose máu]
```
...
![Cấu trúc phân nhánh của glycogen](https://commons.wikimedia.org/wiki/Special\:FilePath/Glycogen_structure.png)
...
```mermaid
flowchart LR
    A[Ăn nhiều carbohydrate hơn] --> B[Tăng dự trữ glycogen]
    B --> C[Giữ thêm nước trong cơ]
    C --> D[Cân nặng có thể tăng nhanh]
    C --> E[Cơ nhìn đầy và căng hơn]

    F[Giảm mạnh carbohydrate] --> G[Giảm glycogen]
    G --> H[Mất bớt nước đi kèm]
    H --> I[Cân nặng giảm nhanh ban đầu]
    H --> J[Cơ có thể nhìn xẹp hơn]
```
...
![Minh họa chất xơ trong hệ tiêu hóa](https://commons.wikimedia.org/wiki/Special\:FilePath/Ingestion_of_Dietary_Fiber.svg)
...
![Ảnh hưởng của insulin lên hấp thu và chuyển hóa glucose](https://commons.wikimedia.org/wiki/Special\:FilePath/Insulin_glucose_metabolism.jpg)
...
```mermaid
flowchart LR
    A[Ăn carbohydrate] --> B[Glucose máu tăng]
    B --> C[Tuyến tụy tiết insulin]
    C --> D[Tế bào cơ tăng hấp thu glucose]
    C --> E[Tăng tổng hợp glycogen]
    C --> F[Giảm giải phóng glucose từ gan]
    C --> G[Ức chế phân giải mô dự trữ]
    D --> H[Glucose máu trở về mức phù hợp]
    E --> H
    F --> H
```
...
```mermaid
flowchart TD
    A[Insulin tăng sau bữa ăn] --> B{Có thặng dư calorie kéo dài không?}
    B -- Không --> C[Không tự động tăng mỡ]
    B -- Có --> D[Năng lượng dư được dự trữ]
    D --> E[Lượng mỡ cơ thể tăng theo thời gian]
```
