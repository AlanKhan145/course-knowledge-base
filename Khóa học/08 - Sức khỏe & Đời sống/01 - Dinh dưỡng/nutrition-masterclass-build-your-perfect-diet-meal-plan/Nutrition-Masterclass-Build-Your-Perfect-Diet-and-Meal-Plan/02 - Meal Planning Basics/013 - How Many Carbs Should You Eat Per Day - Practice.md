...
```mermaid
flowchart LR
    A[1. Xác định tổng kcal/ngày] --> B[2. Tính protein]
    B --> C[3. Tính fat]
    C --> D[4. Phần kcal còn lại]
    D --> E[5. Chia 4 để ra gram carb]
```
...
```mermaid
flowchart TD
    A[Mục tiêu thay đổi calorie] --> B{Đang cắt hay tăng?}
    B -->|Giảm mỡ| C[Cắt carb trước]
    B -->|Tăng cơ| D[Tăng carb trước]

    C --> E[Giữ protein]
    C --> F[Giữ fat ở/gần mức sàn]
    D --> G[Giữ protein]
    D --> H[Giữ fat đủ dùng]
    D --> I[Tăng carb để hỗ trợ tập luyện và phục hồi]
```
...
```markdown
![Nguồn carb lành mạnh](https://images.unsplash.com/photo-1515543904379-3d757afe72e1?auto=format&fit=crop&w=1200&q=80)
```
...
```markdown
![Bữa ăn giàu carb cho người tập](https://images.unsplash.com/photo-1490645935967-10de6ba17061?auto=format&fit=crop&w=1200&q=80)
```
...
```markdown
![Yến mạch và ngũ cốc nguyên hạt](https://images.unsplash.com/photo-1515003197210-e0cd71810b5f?auto=format&fit=crop&w=1200&q=80)
```
...
```markdown
![Khoai, gạo và trái cây](https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=1200&q=80)
```
...
```mermaid
flowchart LR
    A[Tổng kcal/ngày] --> B[Đặt protein]
    B --> C[Đặt fat]
    C --> D[Tính kcal còn lại]
    D --> E[Chia 4 để ra gram carb]
```
