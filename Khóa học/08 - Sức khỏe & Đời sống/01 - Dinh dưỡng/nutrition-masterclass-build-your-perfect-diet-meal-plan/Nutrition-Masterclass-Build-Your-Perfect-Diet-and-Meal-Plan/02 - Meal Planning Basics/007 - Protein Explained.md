[![Їжте на сніданок, обід та вечерю. Навіщо людині білки: користь, норма та білкові дієти | Новини України | LIGA.net](https://images.openai.com/static-rsc-4/BoUKP1M_JOUueZroTWK3zKkvGEMTzoOSxb4Oge1fsODlXaz_azmHG_yNM5UKhQkT0CeAu7riaCKgsbR9TIqfDTY5j4sfoEaVomlVVgYQdWPS7R9xy85Hu3vExQBUPDtLrgMEMtUb-3y6080YTwNlPjhRk7bZh0hw92OteUmFm2M?purpose=inline)](https://life.liga.net/poyasnennya/article/eshte-na-zavtrak-obed-i-ujin-zachem-cheloveku-belki-polza-norma-i-belkovye-diety?utm_source=chatgpt.com)
...
```mermaid
flowchart TD
    A[Acid amin trong cơ thể] --> B[Acid amin thiết yếu — EAA]
    A --> C[Acid amin không thiết yếu]
    A --> D[Acid amin thiết yếu có điều kiện]

    B --> B1[Cơ thể không tự tạo đủ]
    B --> B2[Phải lấy từ thức ăn]

    C --> C1[Cơ thể có thể tự tổng hợp]

    D --> D1[Nhu cầu có thể tăng khi bệnh tật]
    D --> D2[Chấn thương, stress hoặc tăng trưởng]
```
...
[![Cấu trúc các acid amin](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Molecular%20structures%20of%20the%2021%20proteinogenic%20amino%20acids.svg)](https://commons.wikimedia.org/wiki/File:Molecular_structures_of_the_21_proteinogenic_amino_acids.svg)
...
```mermaid
flowchart LR
    A[Thực phẩm chứa protein] --> B[Tiêu hóa tại dạ dày và ruột]
    B --> C[Protein được phân giải]
    C --> D[Peptide và acid amin]
    D --> E[Hấp thu vào máu]
    E --> F[Kho acid amin khả dụng]
    F --> G1[Xây dựng và sửa chữa mô]
    F --> G2[Tạo enzyme]
    F --> G3[Tạo hormone và chất dẫn truyền]
    F --> G4[Tạo kháng thể]
    F --> G5[Tổng hợp protein cơ]
```
...
```mermaid
flowchart LR
    A[Tập kháng lực] --> B[Tín hiệu thích nghi]
    C[Protein trong chế độ ăn] --> D[Acid amin thiết yếu]
    B --> E[Tổng hợp protein cơ]
    D --> E
    E --> F[Phục hồi và tăng trưởng cơ]
```
...
[![Cấu trúc leucine](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Leucine.svg)](https://commons.wikimedia.org/wiki/File:Leucine.svg)
...
```mermaid
flowchart LR
    A[Bữa ăn chứa protein] --> B[Tăng acid amin trong máu]
    B --> C[Tăng leucine khả dụng]
    C --> D[Kích hoạt tín hiệu mTORC1]
    D --> E[Khởi động quá trình dịch mã]
    E --> F[Tăng tổng hợp protein cơ — MPS]
```
...
```mermaid
flowchart TD
    A[Tổng protein cần trong ngày] --> B[Chia thành 3–5 lần ăn]
    B --> C1[Bữa sáng]
    B --> C2[Bữa trưa]
    B --> C3[Bữa phụ hoặc sau tập]
    B --> C4[Bữa tối]
    B --> C5[Bữa trước ngủ nếu cần]

    C1 --> D[Khoảng 20–40 g/lần]
    C2 --> D
    C3 --> D
    C4 --> D
    C5 --> D
...
[![Các nguồn thực phẩm giàu protein](https://commons.wikimedia.org/wiki/Special\:Redirect/file/Protein-rich%20Foods.jpg)](https://commons.wikimedia.org/wiki/File:Protein-rich_Foods.jpg)
...
```mermaid
flowchart LR
    A[Chọn nguồn protein] --> B[Thêm rau hoặc trái cây]
    B --> C[Thêm nguồn carbohydrate]
    C --> D[Thêm chất béo phù hợp]
    D --> E[Điều chỉnh khẩu phần theo calorie]
```
