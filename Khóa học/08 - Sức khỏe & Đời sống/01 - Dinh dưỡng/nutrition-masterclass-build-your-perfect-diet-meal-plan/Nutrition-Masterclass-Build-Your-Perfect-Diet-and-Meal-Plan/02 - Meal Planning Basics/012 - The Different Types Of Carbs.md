...
```mermaid
flowchart TD
    A[Carbohydrate] --> B[Monosaccharide<br/>Một đơn vị đường]
    A --> C[Disaccharide<br/>Hai đơn vị đường]
    A --> D[Polysaccharide<br/>Chuỗi nhiều đơn vị đường]

    B --> B1[Glucose]
    B --> B2[Fructose]
    B --> B3[Galactose]

    C --> C1[Sucrose<br/>Glucose + Fructose]
    C --> C2[Lactose<br/>Glucose + Galactose]
    C --> C3[Maltose<br/>Glucose + Glucose]
...
![Sơ đồ phân loại carbohydrate](https://upload.wikimedia.org/wikipedia/commons/0/0b/Carbohydrates.jpeg)
...
```mermaid
flowchart LR
    A[Chất lượng nguồn carb] --> B[Hàm lượng chất xơ]
    A --> C[Mức độ chế biến]
    A --> D[Protein và chất béo đi kèm]
    A --> E[Vitamin và khoáng chất]
    A --> F[Mật độ năng lượng]
    A --> G[Kích thước khẩu phần]
    A --> H[Khả năng tiêu hóa cá nhân]
```
...
![Đồ thị minh họa phản ứng đường huyết sau khi ăn](https://upload.wikimedia.org/wikipedia/commons/6/6f/Glykemiskt_Index.PNG)
...
```mermaid
flowchart LR
    A[GI của thực phẩm] --> D[Tính GL]
    B[Lượng carb tiêu hóa được] --> D
    C[Kích thước khẩu phần] --> B
    D --> E[Ước tính tác động đường huyết<br/>của khẩu phần thực tế]
```

### 5.2. Ví dụ với dưa hấu

![Dưa hấu có nhiều nước và lượng carbohydrate trong một khẩu phần tương đối thấp](https://upload.wikimedia.org/wikipedia/commons/b/b9/Watermelon.jpg)
...
```mermaid
flowchart TD
    A[Cơm ăn riêng] --> B[Phản ứng đường huyết A]

    C[Cơm + thịt/cá + rau + chất béo] --> D[Tiêu hóa chậm hơn]
    D --> E[Phản ứng đường huyết khác A]
```
...
![So sánh chất xơ hòa tan và không hòa tan](https://sunfiber.com/wp-content/uploads/2022/12/soluble-fiber-vs-insoluble-fiber.png)
...
```mermaid
flowchart LR
    A[Trái cây nguyên quả] --> B[Nhai]
    B --> C[Chất xơ còn nguyên]
    C --> D[Ăn chậm hơn và no tốt hơn]

    E[Nước ép] --> F[Ít hoặc mất chất xơ]
    F --> G[Uống nhanh]
    G --> H[Dễ nạp nhiều đường và calorie]
```
...
```mermaid
flowchart TD
    A[Năng lượng nạp vào] --> C{So sánh với năng lượng tiêu hao}
    B[Năng lượng tiêu hao] --> C

    C -->|Nạp cao hơn kéo dài| D[Tăng cân]
    C -->|Tương đương| E[Duy trì cân nặng]
    C -->|Nạp thấp hơn kéo dài| F[Giảm cân]
```
...
```mermaid
flowchart TD
    A[Chọn một thực phẩm chứa carb] --> B{Có giữ cấu trúc<br/>tự nhiên tương đối không?}

    B -->|Có| C[Kiểm tra chất xơ và vi chất]
    B -->|Không nhiều| D[Kiểm tra mức tinh chế<br/>và đường bổ sung]

    C --> E{Phù hợp tiêu hóa<br/>và sở thích không?}
    D --> F{Khẩu phần và tần suất<br/>có hợp lý không?}

    E -->|Có| G[Đưa vào chế độ ăn]
    E -->|Không| H[Chọn nguồn khác]

    F -->|Có| G
    F -->|Không| I[Giảm khẩu phần hoặc tần suất]

    G --> J[Đánh giá trong bối cảnh<br/>tổng calorie, protein và chất xơ]
```
