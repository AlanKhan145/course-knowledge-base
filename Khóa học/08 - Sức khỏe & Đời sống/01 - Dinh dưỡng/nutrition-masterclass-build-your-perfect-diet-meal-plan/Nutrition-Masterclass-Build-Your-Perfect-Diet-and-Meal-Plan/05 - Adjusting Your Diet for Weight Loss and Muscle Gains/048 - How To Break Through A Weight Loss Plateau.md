...
![Cân điện tử dùng để theo dõi cân nặng](https://commons.wikimedia.org/wiki/Special\:FilePath/Fitness%20digital%20scale%20weight%20loss%20%2833837094584%29.jpg)
...
```mermaid
flowchart LR
    A[Cân nặng trên cân] --> B[Mỡ cơ thể]
    A --> C[Khối cơ và mô nạc]
    A --> D[Glycogen]
    A --> E[Nước trong cơ thể]
    A --> F[Thức ăn và chất lỏng<br/>trong hệ tiêu hóa]
```
...
```mermaid
flowchart LR
    A[Ăn nhiều carbohydrate] --> B[Tăng dự trữ glycogen]
    B --> C[Tăng lượng nước đi kèm]
    C --> D[Cân tăng tạm thời]
    D --> E[Không nhất thiết tăng mỡ]
```
...
```mermaid
flowchart TD
    A[Giảm cân] --> B[Cơ thể nhẹ hơn]
    A --> C[Ăn ít hơn]
    B --> D[Tiêu hao khi vận động giảm]
    C --> E[Hiệu ứng nhiệt của thức ăn giảm]
    A --> F[Có thể xuất hiện<br/>thích nghi chuyển hóa]
    D --> G[TDEE mới thấp hơn]
    E --> G
    F --> G
    G --> H[Mức calorie cũ không còn<br/>tạo thâm hụt đủ lớn]
```
...
```mermaid
flowchart TD
    A[Cân không giảm] --> B{Đã theo dõi trung bình<br/>ít nhất 2–3 tuần?}

    B -- Chưa --> C[Tiếp tục cân trong điều kiện nhất quán]
    B -- Rồi --> D{Vòng eo hoặc ảnh tiến độ<br/>có thay đổi không?}

    D -- Có --> E[Có thể vẫn đang giảm mỡ<br/>Tiếp tục kế hoạch]
    D -- Không --> F[Kiểm tra calorie ẩn và khẩu phần]

    F --> G{Mức tuân thủ có ổn định?}
    G -- Không --> H[Sửa cách theo dõi và môi trường ăn uống]
    G -- Có --> I[Tính lại nhu cầu năng lượng hiện tại]

    I --> J{Khả năng phục hồi<br/>và cảm giác đói có ổn?}
    J -- Có --> K[Giảm nhẹ calorie hoặc tăng vận động]
    J -- Không --> L[Cân nhắc giai đoạn duy trì có kế hoạch]

    K --> M[Theo dõi thêm 2 tuần]
    L --> M
```
...
![Minh họa đo vòng eo để theo dõi thay đổi hình thể](https://commons.wikimedia.org/wiki/Special\:FilePath/Man%20Measuring%20His%20Waist%20Cartoon.svg)
...
```mermaid
flowchart LR
    A[Chuẩn hóa việc cân] --> B[Tính trung bình 7 ngày]
    B --> C[Theo dõi 2–3 tuần]
    C --> D[Kiểm tra vòng eo và ảnh]
    D --> E[Kiểm toán calorie]
    E --> F[Tính lại nhu cầu]
    F --> G[Điều chỉnh nhẹ]
    G --> H[Đánh giá lại sau 2 tuần]
```
