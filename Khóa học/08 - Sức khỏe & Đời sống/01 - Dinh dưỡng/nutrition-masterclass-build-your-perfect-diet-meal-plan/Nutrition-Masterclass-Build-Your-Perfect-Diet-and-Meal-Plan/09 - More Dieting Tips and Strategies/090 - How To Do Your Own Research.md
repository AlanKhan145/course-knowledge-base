...
[![Healthy Eating Plate - Harvard Health](https://tse4.mm.bing.net/th/id/OIP.dFCxYiREcMCK1mdjP_UrtAHaFu?r=0\&pid=Api)](https://www.health.harvard.edu/staying-healthy/healthy-eating-plate?utm_source=chatgpt.com) 
...
![Thực phẩm đa dạng và giàu dinh dưỡng](https://upload.wikimedia.org/wikipedia/commons/6/6d/Good_Food_Display_-_NCI_Visuals_Online.jpg)
...
```mermaid
flowchart LR
    A[Mục tiêu rõ ràng] --> B[Chế độ ăn phù hợp]
    B --> C[Thói quen hằng ngày]
    C --> D[Theo dõi tiến triển]
    D --> E[Điều chỉnh khi cần]
    E --> C
```
...
![Lập kế hoạch và nghiên cứu chế độ ăn](https://upload.wikimedia.org/wikipedia/commons/b/bc/Healthy_meal_planning_with_fresh_fruits_and_vegetables_in_a_bright_kitchen_setting.jpg)
...
```mermaid
flowchart TD
    A[Năng lượng từ thức ăn và đồ uống] --> C[Cân bằng năng lượng]
    B[Năng lượng cơ thể tiêu hao] --> C
    C -->|Nạp lớn hơn tiêu hao| D[Xu hướng tăng dự trữ năng lượng]
    C -->|Gần bằng nhau| E[Xu hướng duy trì]
    C -->|Nạp thấp hơn tiêu hao| F[Xu hướng giảm khối lượng cơ thể]
```
...
![Tháp mức độ bằng chứng](https://upload.wikimedia.org/wikipedia/commons/0/0d/Level_of_evidence_pyramid.png)
...
```mermaid
flowchart TD
    A[Tuyên bố dinh dưỡng mới] --> B{Phù hợp nguyên tắc nền tảng?}
    B -->|Không| C[Cảnh báo lớn]
    B -->|Có| D{Lời hứa có thực tế?}
    D -->|Không| E[Khả năng quảng cáo phóng đại]
    D -->|Có| F{Có quy tắc tuyệt đối hoặc quá phức tạp?}
    F -->|Có| G[Yêu cầu bằng chứng mạnh hơn]
    F -->|Không| H[Tìm tổng quan và nghiên cứu]
    H --> I[Đánh giá thiết kế, đối tượng và kết quả]
    I --> J{Lợi ích lớn hơn rủi ro và chi phí?}
    J -->|Có| K[Thử nghiệm có kiểm soát và theo dõi]
    J -->|Không rõ| L[Chưa áp dụng hoặc hỏi chuyên gia]
```
