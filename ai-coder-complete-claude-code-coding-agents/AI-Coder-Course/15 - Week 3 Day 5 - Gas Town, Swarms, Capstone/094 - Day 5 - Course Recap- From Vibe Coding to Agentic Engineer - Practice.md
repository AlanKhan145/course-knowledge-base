...
```mermaid
flowchart TD
    A[Week 1: Vibe Coding] --> B[AI IDEs and Coding Assistants]
    B --> C[YOLO Mode and MVP Building]
    C --> D[Week 2: Professional Workflows]
    D --> E[Claude Code, OpenCode, AMP]
    E --> F[Commands, Checkpoints, Slash Commands]
    F --> G[Jira Workflow and Legal SaaS Platform]
    G --> H[Week 3: Agentic Engineering Frontier]
    H --> I[Multi-Agent Systems and Sub-Agents]
    I --> J[Hooks, Sandboxing, Remote Execution]
    J --> K[Large Codebase Workflows]
    K --> L[Claude Agent SDK, Cowork, OpenClaw]
    L --> M[Orchestrators: GSD, Agent Teams, Gastown]
    M --> N[Capstone: Finance Workstation]
    N --> O[Agentic Engineer]
```
...
```mermaid
flowchart LR
    A[Vibe Coder] --> B[Vibe Engineer]
    B --> C[Agentic Engineer]

    A --> A1[Simple prompts<br/>Fast experiments<br/>AI-assisted coding]
    B --> B1[Professional workflows<br/>Git, Jira, checkpoints<br/>Structured builds]
    C --> C1[Multi-agent orchestration<br/>Sandboxing<br/>Capstone systems]
```
