...
flowchart LR
    ...
    D --> E[Anger]
    E --> F[Acceptance]
    F --> G[Better Workflows]
    G --> H[Professional Agentic Coding]
```
...
```mermaid id="t1x4po"
flowchart TD
    A[AI Coding Terminology] --> B[Vibe Coding]
    A --> C[Vibe Engineering]
    A --> D[Agentic Coder]
    A --> E[Coding Agent]

    B --> B1[Fast creative prompting]
    B --> B2[Prototype-first workflow]

    C --> C1[Professional AI-assisted engineering]
    C --> C2[Process, review, testing]

    D --> D1[Human developer using agents]
    D --> D2[Sometimes means agent builder]

    E --> E1[AI system that writes code]
    E --> E2[Examples: Cursor, Claude Code, Codex]
```
...
```mermaid id="tvnmwq"
flowchart TD
    A[Coding Agents] --> B[IDE Interface]
    A --> C[Plugin / Extension Interface]
    A --> D[CLI Interface]
...
```mermaid id="kq6eqc"
flowchart LR
    A[AI as Assistant] --> B[Human gives small request]
    B --> C[AI suggests or explains]
    C --> D[Human decides what to use]

    E[AI as Autonomous Agent] --> F[Human gives larger goal]
    F --> G[AI plans and edits files]
    G --> H[AI may run tools or commands]
    H --> I[Human reviews and guides]
```
