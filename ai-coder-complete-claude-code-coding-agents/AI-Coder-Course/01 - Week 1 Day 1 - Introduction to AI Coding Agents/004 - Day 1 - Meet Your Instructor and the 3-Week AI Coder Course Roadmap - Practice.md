...
```mermaid id="249agv"
flowchart TD
    A[AI Coder Course] --> B[Core Skills]
    A --> C[Platforms & Products]
    A --> D[Commercial Projects]
...
```mermaid id="z5l6dw"
flowchart LR
    A[Week 1: Vibe Coding] --> B[Week 2: Vibe Engineering]
    B --> C[Week 3: Agentic Engineering]

    A --> A1[Foundations]
    A --> A2[Tooling]
    A --> A3[Simple Projects]

    B --> B1[CLI Coding]
    B --> B2[MCP, Skills, Plugins]
    B --> B3[Debugging and Team Workflows]

    C --> C1[Multi-Agent Systems]
    C --> C2[Sandboxing]
    C --> C3[Large Codebases]
    C --> C4[Agent Swarms and Capstone]
```
...
```mermaid id="m5db1x"
flowchart TD
    A[Start: Curious Learner] --> B[Week 1: Vibe Coding]
    B --> C[Can Build Quickly with AI Agents]
    C --> D[Week 2: Vibe Engineering]
    D --> E[Can Use Professional Agent Workflows]
    E --> F[Week 3: Agentic Engineering]
    F --> G[Can Build Large-Scale Software with Teams of Agents]
    G --> H[Outcome: Expert Agentic Engineer]
```
