...
```mermaid
flowchart LR
    A[Adjective<br/>Tính từ] --> B[Noun<br/>Danh từ]
    B --> C[a big house]
    B --> D[a small bag]
    B --> E[a beautiful beach]
```
...
```mermaid
flowchart TD
    A[Opposite Adjectives] --> B[big ↔ small]
    A --> C[new ↔ old]
    A --> D[good ↔ bad]
    A --> E[easy ↔ difficult]
    A --> F[fast ↔ slow]
...
```mermaid
flowchart TD
    A[Adjective] --> B[be + adjective]
    A --> C[look + adjective]
    A --> D[feel + adjective]

    B --> E[The room is small.]
    C --> F[The room looks small.]
    D --> G[I feel cold.]
```
...
```mermaid
flowchart TD
    A[ADJECTIVE<br/>Tính từ] --> B[Trước danh từ]
    A --> C[Sau linking verb]
    A --> D[Mức độ]
    A --> E[Cặp trái nghĩa]

    B --> B1[a big house]
    B --> B2[a beautiful beach]
