...
```mermaid
flowchart TD
    A[Muốn giới thiệu nguồn gốc] --> B{Bạn muốn nói gì?}

    B -->|Đến từ đâu?| C[COUNTRY]
    B -->|Mang quốc tịch gì?| D[NATIONALITY]

    C --> E[be + from + country]
    E --> F[I am from Vietnam.]
...
```mermaid
flowchart LR
    A[COUNTRY<br>Quốc gia] --> B[be from]
    B --> C[I am from Vietnam.]

    D[NATIONALITY<br>Quốc tịch] --> E[be]
    E --> F[I am Vietnamese.]

    G[PLACE<br>Nơi sống] --> H[live in]
    H --> I[I live in Hanoi.]
```
